    ...
    <div class="page-header">
      <div class="header-content">
        <div class="title-section">
          ...
          <p class="page-description">期货投研概览，提供期货数据的综合视图和分析工具</p>
        </div>
      </div>
    </div>
      ...
      <el-row :gutter="16" class="stats-row">
        <el-col :xs="24" :sm="12" :md="8" :lg="6">
          <el-card shadow="hover" class="stat-card">
            <el-statistic title="期货合约总数" :value="stats.total_contracts || 0" />
          </el-card>
        </el-col>
        <el-col :xs="24" :sm="12" :md="8" :lg="6">
          <el-card shadow="hover" class="stat-card">
        ...
        <el-row :gutter="16">
          <el-col :xs="24" :sm="12" :md="8">
            <el-button
              ...
              size="large"
              style="width: 100%"
              @click="$router.push('/futures/collections')"
            >
              <el-icon><Box /></el-icon>
          ...
          <el-col :xs="24" :sm="12" :md="8" style="margin-top: 8px">
            <el-button
              ...
              size="large"
              style="width: 100%"
              @click="$router.push('/futures/analysis')"
            >
              <el-icon><DataAnalysis /></el-icon>
              <span style="margin-left: 8px;">期货分析</span>
            </el-button>
          </el-col>
        </el-row>
      </el-card>
      ...
      <el-card shadow="hover" style="margin-top: 16px;">
        ...
        <el-descriptions :column="1" border>
          <el-descriptions-item label="数据集合">
            查看和管理各类期货数据集合，包括基础信息、日行情、主力合约等
          </el-descriptions-item>
          <el-descriptions-item label="期货分析">
            对期货合约进行深入分析，包括价格走势、持仓量变化等
          </el-descriptions-item>
...
const loadData = async () => {
  ...
  try {
    // 加载概览数据
    const overviewRes = await futuresApi.getOverview()
    if (overviewRes.success) {
      stats.value = overviewRes.data
    }

    // 加载集合列表
    const collectionsRes = await futuresApi.getCollections()
