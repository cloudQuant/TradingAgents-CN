<template>
  <div class="collection-page">
    ...
    <div class="content">
      ...
      <el-tabs type="border-card" class="analysis-tabs" v-if="hasCharts">
        <el-tab-pane label="结构分布分析">
          <el-row :gutter="24">
            <el-col :span="12">
              <div class="chart-title">债券类型分布</div>
    ...
    <CollectionOverviewDialog
      ...
      :latest-update="stats?.latest_date"
...
import { useBondStore } from '@/stores/bonds'
...
const hasCharts = computed(() => {
  return stats.value?.grade_stats && stats.value.grade_stats.length > 0
})
...
const categoryDistribution = computed(() => {
  if (!stats.value || !stats.value.grade_stats) return []
  
  const total = stats.value.grade_stats.reduce((sum: number, item: any) => sum + item.count, 0)
  
  return stats.value.grade_stats.map((item: any, index: number) => ({
    category: item.grade || '未分类',
    count: item.count,
...
const typePieOption = computed(() => {
  if (!stats.value || !stats.value.grade_stats) return null
  
  const chartData = stats.value.grade_stats.map((item: any, index: number) => ({
    name: item.grade || '未分类',
    value: item.count,
