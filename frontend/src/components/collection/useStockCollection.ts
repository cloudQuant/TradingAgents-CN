...
export function useStockCollection() {
  ...
  const collectionInfo = ref<StockCollection | null>(null)
  ...
  const loadData = async (extraParams: Record<string, any> = {}) => {
    loading.value = true
    try {
      const collectionsRes = await stocksApi.getCollections()
      ...
      const statsRes = await stocksApi.getCollectionStats(collectionName.value)
      if (statsRes.success && statsRes.data) {
        stats.value = statsRes.data
      }

      const dataRes = await stocksApi.getCollectionData(collectionName.value, {
        page: page.value,
  ...
  const exportAllData = async (
    ...
    try {
      ...
      const blob = await stocksApi.exportCollectionData(collectionName.value, payload)
  ...
  const loadUpdateConfig = async () => {
    try {
      ...
      const res = await stocksApi.getUpdateConfig(collectionName.value)
  ...
  const pollTaskStatus = async () => {
    ...
    batchProgressTimer = setInterval(async () => {
      try {
        ...
        const res = await stocksApi.getRefreshTaskStatus(currentTaskId.value)
        ...
        if (res.success && res.data) {
          ...
          if (task.result) {
            ...
            if (task.result.saved !== undefined && task.result.saved > 0) {
              parts.push(`保存 ${task.result.saved.toLocaleString()} 条数据`)
            }
            if (parts.length > 0) {
  ...
  const handleSingleUpdate = async () => {
    ...
    try {
      ...
      const res = await stocksApi.refreshCollection(collectionName.value, processedParams)
  ...
  const handleBatchUpdate = async () => {
    ...
    try {
      ...
      const res = await stocksApi.refreshCollection(collectionName.value, params)
  ...
  const handleClearData = async () => {
    ...
    clearing.value = true
    try {
      const res = await stocksApi.clearCollectionData(collectionName.value)
      if (res.success && res.data) {
