...
export function useOptionsCollection() {
  ...
  const collectionInfo = ref<OptionsCollection | null>(null)
  ...
  const loadData = async (extraParams: Record<string, any> = {}) => {
    ...
    try {
      ...
      const statsRes = await optionsApi.getCollectionStats(collectionName.value)
      if (statsRes.success && statsRes.data) {
        stats.value = statsRes.data as any
      }

      const dataRes = await optionsApi.getCollectionData(collectionName.value, {
        page: page.value,
  ...
  const loadUpdateConfig = async () => {
    try {
      ...
      const res = await optionsApi.getCollectionUpdateConfig(collectionName.value)
  ...
  const pollTaskStatus = async () => {
    ...
    batchProgressTimer = setInterval(async () => {
      ...
      try {
        const res = await optionsApi.getRefreshTaskStatus(collectionName.value, currentTaskId.value)
        if (res.success && res.data) {
          const task = res.data
  ...
  const handleSingleUpdate = async () => {
    ...
    try {
      ...
      const res = await optionsApi.refreshCollectionData(collectionName.value, params)
