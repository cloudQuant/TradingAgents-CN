...
export function useFuturesCollection() {
  ...
  const collectionInfo = ref<FuturesCollection | null>(null)
  ...
  const loadData = async (extraParams: Record<string, any> = {}) => {
    ...
    try {
      ...
      const statsRes = await futuresApi.getCollectionStats(collectionName.value)
      if (statsRes.success && statsRes.data) {
        stats.value = statsRes.data as any
      }

      const dataRes = await futuresApi.getCollectionData(collectionName.value, {
        page: page.value,
  ...
  const handleSingleUpdate = async () => {
    ...
    try {
      ...
      const res = await futuresApi.refreshCollectionData(collectionName.value, params)
