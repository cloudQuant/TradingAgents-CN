...
负责扫描 indexs/providers 目录，加载所有 Provider 类，
并为其附加必要的元信息。
"""
...
from app.services.data_sources.indexs.collection_metadata import INDEX_COLLECTION_METADATA
from app.config.index_update_config import INDEX_UPDATE_CONFIGS

PROVIDERS_PACKAGE = "app.services.data_sources.indexs.providers"
...
def _apply_metadata(provider_cls: Type[BaseProvider]):
    ...
    config = INDEX_UPDATE_CONFIGS.get(provider_cls.collection_name, {})
    meta = INDEX_COLLECTION_METADATA.get(provider_cls.collection_name, {})
...
@lru_cache()
def get_registered_index_providers() -> List[Type[BaseProvider]]:
    """获取所有 Index Provider 类"""
...
def get_provider_class(collection_name: str) -> Optional[Type[BaseProvider]]:
    """根据集合名称获取 Provider 类"""
    for provider_cls in get_registered_index_providers():
