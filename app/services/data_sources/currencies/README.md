...
| `app/routers/currencies.py` | API 路由定义 | `list_currencies_collections()`, `refresh_currency_collection()`, `get_currency_refresh_task_status()` |
...
└─────────────────────────────────────────────────────────────────────────────┘
                                       ↓
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Service 层 CurrencyLatestService 等                        │
├─────────────────────────────────────────────────────────────────────────────┤
│  update_batch_data()                                                         │
│    1. 获取参数（api_key 等）                                                  │
...
└─────────────────────────────────────────────────────────────────────────────┘
                                       ↓
┌─────────────────────────────────────────────────────────────────────────────┐
│                   Provider 层 CurrencyLatestProvider 等                       │
├─────────────────────────────────────────────────────────────────────────────┤
...
1. **创建 Service**- `services/{collection_name}_service.py`
...
1. **注册 Service**- `app/services/currency_refresh_service.py`
...
1. **参数过滤**: 前端特有参数（如 `update_type`, `batch`）会被自动过滤
2. **进度更新**: 批量更新时，Service 层负责更新进度和完成任务
3. **错误处理**: 单条更新失败时，由 RefreshService 处理任务状态
