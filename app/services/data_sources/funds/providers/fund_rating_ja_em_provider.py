"""
基金评级-济安金信-东财数据提供者（重构版：继承BaseProvider，需要季度日期参数）
"""
import pandas as pd
...
class FundRatingJaEmProvider(BaseProvider):
    """基金评级-济安金信-东财数据提供者（需要季度日期参数）"""

    collection_description = "东方财富网-基金评级-济安金信评级（需要季度日期参数，格式：YYYY-MM-DD）"
    collection_route = "/funds/collections/fund_rating_ja_em"
    collection_order = 50

    collection_name = "fund_rating_ja_em"
    display_name = "济安金信评级-东财"
    akshare_func = "fund_rating_ja"  # 注意：接口名称是 fund_rating_ja，不是 fund_rating_ja_em
    unique_keys = ["代码", "日期"]
    ...
    field_info = [
        ...
        {"name": "来源", "type": "string", "description": "来源接口: fund_rating_ja"},
    ]
