"""
基金行业配置-东财数据提供者（重构版：继承BaseProvider）
"""
import pandas as pd
from app.services.data_sources.base_provider import BaseProvider


class FundPortfolioIndustryAllocationEmProvider(BaseProvider):
    """基金行业配置-东财数据提供者"""

    collection_description = "东方财富网-数据中心-行业配置（需要基金代码和日期，支持单个/批量更新）"
    collection_route = "/funds/collections/fund_portfolio_industry_allocation_em"
    collection_order = 45
        ...
        try:
            ...
            if date_param:
                ...
                if len(year) >= 4 and year[:4].isdigit():
                    ...
                    # 如果无法提取，尝试从截止时间字段中提取
                    if "截止时间" in df.columns and not df.empty:
                        # 从第一条记录的截止时间字段中提取年份
                        first_date = str(df.iloc[0]["截止时间"]) if not df.empty else ""
                ...
                # 如果没有date参数，尝试从截止时间字段中提取
                if "截止时间" in df.columns and not df.empty:
                    # 从截止时间字段中提取年份
                    df["年份"] = df["截止时间"].apply(
                        lambda d: str(d)[:4] if len(str(d)) >= 4 and str(d)[:4].isdigit() else ""
                    )
                else:
                    df["年份"] = ""
