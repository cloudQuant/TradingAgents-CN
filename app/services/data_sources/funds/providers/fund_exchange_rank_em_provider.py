"""
场内基金排行-东财数据提供者（重构版：继承SimpleProvider，无参数接口）
"""
import pandas as pd
...
class FundExchangeRankEmProvider(SimpleProvider):
    """场内基金排行-东财数据提供者（无参数接口，直接获取所有数据）"""

    collection_description = "东方财富网-数据中心-场内交易基金排行榜，每个交易日17点后更新"
    collection_route = "/funds/collections/fund_exchange_rank_em"
    collection_order = 31
    ...
    def fetch_data(self, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            # 不传递任何参数给 akshare（fund_exchange_rank_em 接口不需要参数）
