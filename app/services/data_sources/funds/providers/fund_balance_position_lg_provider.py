"""
平衡混合型基金仓位-乐咕乐股数据提供者（重构版：继承SimpleProvider，无参数接口）
"""
import pandas as pd
...
class FundBalancePositionLgProvider(SimpleProvider):
    """平衡混合型基金仓位-乐咕乐股数据提供者（无参数接口，直接获取所有数据）"""

    collection_description = "乐咕乐股-基金仓位-平衡混合型基金仓位（无需参数，支持更新所有）"
    collection_route = "/funds/collections/fund_balance_position_lg"
    collection_order = 67
    ...
    def fetch_data(self, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            # 不传递任何参数给 akshare（fund_balance_position_lg 接口不需要参数）
