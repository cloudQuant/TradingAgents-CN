"""
基金报告行业配置-巨潮数据提供者（重构版：继承BaseProvider，需要date参数）
"""
import pandas as pd
...
class FundReportIndustryAllocationCninfoProvider(BaseProvider):
    """基金报告行业配置-巨潮数据提供者（需要日期参数）"""

    collection_description = "巨潮资讯-数据中心-专题统计-基金报表-基金行业配置（需要日期参数，格式：YYYYMMDD，如 20210630）"
    collection_route = "/funds/collections/fund_report_industry_allocation_cninfo"
    collection_order = 62
    ...
    field_info = [
        {"name": "行业编码", "type": "string", "description": "行业编码"},
        {"name": "证监会行业名称", "type": "string", "description": "证监会行业名称"},
        {"name": "报告期", "type": "string", "description": "报告期"},
        {"name": "基金覆盖家数", "type": "int", "description": "基金覆盖家数（只）"},
        {"name": "行业规模", "type": "float", "description": "行业规模（亿元）"},
        {"name": "占净资产比例", "type": "float", "description": "占净资产比例（%）"},
