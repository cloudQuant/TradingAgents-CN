...
参考文档: app/services/data_sources/funds/BATCH_UPDATE_GUIDE.md
"""
...
from ..providers.fund_portfolio_bond_hold_em_provider import FundPortfolioBondHoldEmProvider

logger = logging.getLogger(__name__)


class FundPortfolioBondHoldEmService:
    """基金持仓债券-东财服务"""
    
    def __init__(self, db: AsyncIOMotorClient):
        self.db = db
        self.collection = db["fund_portfolio_bond_hold_em"]
        self.provider = FundPortfolioBondHoldEmProvider()
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"[fund_portfolio_bond_hold_em] update_single_data 收到参数: {kwargs}")
            fund_code = kwargs.get("fund_code") or kwargs.get("symbol") or kwargs.get("code")
            year = kwargs.get("year") or kwargs.get("date")
            logger.info(f"[fund_portfolio_bond_hold_em] 解析参数: fund_code={fund_code}, year={year}")
            ...
            # 单个基金更新：传入基金代码和年份
            logger.info(f"[fund_portfolio_bond_hold_em] 调用 provider.fetch_data(fund_code={fund_code}, year={year})")
            df = self.provider.fetch_data(fund_code=fund_code, year=year)
            
            if df is None or df.empty:
                logger.warning(f"[fund_portfolio_bond_hold_em] provider 返回空数据")
                return {
                    "success": True,
            ...
            extra_fields = {"数据源": "akshare", "接口名称": "fund_portfolio_bond_hold_em"}
            ...
            logger.error(f"[fund_portfolio_bond_hold_em] update_single_data 发生错误: {str(e)}")
            return {
                "success": False,
                "message": str(e),
                "inserted": 0,
    ...
    async def update_batch_data(self, task_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"[fund_portfolio_bond_hold_em] 从 fund_name_em 获取到 {len(fund_codes)} 个基金代码")
            ...
            logger.info(f"[fund_portfolio_bond_hold_em] 年份范围: {years}")

            # 3. 获取已存在的基金代码+年份组合（从季度字段中提取年份）
            if task_manager and task_id:
            ...
            logger.info(f"[fund_portfolio_bond_hold_em] 已存在 {len(existing_combinations)} 个基金代码+年份组合")
            ...
            logger.info(f"[fund_portfolio_bond_hold_em] 需要更新 {total_combinations} 个基金代码+年份组合")
            ...
            async def fetch_and_save(code: str, y: str):
                ...
                async with semaphore:
                    try:
                        ...
                        logger.debug(f"获取基金 {code} 年份 {y} 债券持仓失败: {e}")
                        async with lock:
                            failed += 1
                            processed += 1
            ...
            logger.info(f"[fund_portfolio_bond_hold_em] {message}")
            ...
            logger.error(f"[fund_portfolio_bond_hold_em] 批量更新失败: {e}", exc_info=True)
            if task_manager and task_id:
