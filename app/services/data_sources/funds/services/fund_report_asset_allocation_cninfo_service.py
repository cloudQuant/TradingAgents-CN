"""
基金报告资产配置-巨潮服务（重构版：继承SimpleService，无参数接口，不需要批量更新）
"""
...
from app.services.data_sources.funds.providers.fund_report_asset_allocation_cninfo_provider import FundReportAssetAllocationCninfoProvider
...
class FundReportAssetAllocationCninfoService(SimpleService):
    ...
    extra_metadata = {
        "数据源": "akshare",
        "接口名称": "fund_report_asset_allocation_cninfo",
    }
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        ...
        async with self._update_lock:
            try:
                ...
                # 不传递任何参数给 provider（fund_report_asset_allocation_cninfo 接口不需要参数）
