"""
货币型基金历史行情-东财服务（重构版：继承BaseService）
"""
...
from app.services.data_sources.funds.providers.fund_money_fund_info_em_provider import FundMoneyFundInfoEmProvider
from app.services.database.control_mongodb import ControlMongodb

logger = logging.getLogger(__name__)


class FundMoneyFundInfoEmService(BaseService):
    """货币型基金历史行情-东财服务"""
    
    # ===== 必须定义的属性 =====
    collection_name = "fund_money_fund_info_em"
    provider_class = FundMoneyFundInfoEmProvider
    
    # ===== 可选配置 =====
    time_field = "scraped_at"
    
    # 批量更新配置：从fund_money_fund_daily_em获取基金代码列表
    batch_source_collection = "fund_money_fund_daily_em"
