...
from ..providers.fund_money_fund_info_em_provider import FundMoneyFundInfoEmProvider

logger = logging.getLogger(__name__)


class FundMoneyFundInfoEmService:
    """货币型基金历史行情-东财服务"""
    
    def __init__(self, db: AsyncIOMotorClient):
        self.db = db
        self.collection = db["fund_money_fund_info_em"]
        self.provider = FundMoneyFundInfoEmProvider()
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"[fund_money_fund_info_em] update_single_data 收到参数: {kwargs}")
            fund_code = kwargs.get("fund_code") or kwargs.get("fund") or kwargs.get("code")
            logger.info(f"[fund_money_fund_info_em] 解析参数: fund_code={fund_code}")
            ...
            logger.info(f"[fund_money_fund_info_em] 调用 provider.fetch_data(fund_code={fund_code})")
            df = self.provider.fetch_data(fund_code=fund_code)
            
            if df is None or df.empty:
                logger.warning(f"[fund_money_fund_info_em] provider 返回空数据")
                return {
                    "success": True,
            ...
            extra_fields = {"数据源": "akshare", "接口名称": "fund_money_fund_info_em"}
            ...
            logger.error(f"[fund_money_fund_info_em] update_single_data 发生错误: {str(e)}")
            return {
                "success": False,
                "message": str(e),
                "inserted": 0,
    ...
    async def update_batch_data(self, task_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_manager and task_id:
                task_manager.update_progress(task_id, 0, 100, "正在从 fund_money_fund_daily_em 获取基金代码列表...")
            
            # 1. 从 fund_money_fund_daily_em 获取全部基金代码
            fund_codes: List[str] = []
            cursor = self.db["fund_money_fund_daily_em"].find({}, {"基金代码": 1})
            ...
            if not fund_codes:
                if task_manager and task_id:
                    task_manager.fail_task(task_id, "fund_money_fund_daily_em 集合为空，请先更新货币型基金实时行情")
                return {
                    "success": False,
                    "message": "fund_money_fund_daily_em 集合为空，请先更新货币型基金实时行情",
                    "inserted": 0,
                }
            
            logger.info(f"[fund_money_fund_info_em] 从 fund_money_fund_daily_em 获取到 {len(fund_codes)} 个基金代码")
            ...
            logger.info(f"[fund_money_fund_info_em] 已存在 {len(existing_codes)} 个基金代码")
            ...
            logger.info(f"[fund_money_fund_info_em] 需要更新 {total_codes} 个基金代码")
            ...
            logger.info(f"[fund_money_fund_info_em] {message}")
            ...
            logger.error(f"[fund_money_fund_info_em] 批量更新失败: {e}", exc_info=True)
            if task_manager and task_id:
