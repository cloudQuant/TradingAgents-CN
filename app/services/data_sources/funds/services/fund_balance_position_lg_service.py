"""
平衡混合型基金仓位-乐咕乐股服务（重构版：继承SimpleService，无参数接口，不需要批量更新）
"""
...
from app.services.data_sources.funds.providers.fund_balance_position_lg_provider import FundBalancePositionLgProvider
...
class FundBalancePositionLgService(SimpleService):
    """平衡混合型基金仓位-乐咕乐股服务（无参数接口，直接获取所有数据）"""
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        ...
        async with self._update_lock:
            try:
                ...
                # 不传递任何参数给 provider（fund_balance_position_lg 接口不需要参数）
