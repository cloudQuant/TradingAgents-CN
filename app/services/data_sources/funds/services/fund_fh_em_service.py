...
from ..providers.fund_fh_em_provider import FundFhEmProvider

logger = logging.getLogger(__name__)


class FundFhEmService:
    """基金分红-东财服务"""
    
    def __init__(self, db: AsyncIOMotorClient):
        self.db = db
        self.collection = db["fund_fh_em"]
        self.provider = FundFhEmProvider()
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        """更新单条数据（需要 year 参数）"""
        try:
            logger.info(f"[fund_fh_em] update_single_data 收到参数: {kwargs}")
            year = kwargs.get("year") or kwargs.get("date")
            logger.info(f"[fund_fh_em] 解析参数: year={year}")
            ...
            logger.info(f"[fund_fh_em] 调用 provider.fetch_data(year={year})")
            df = self.provider.fetch_data(year=year)
            
            if df is None or df.empty:
                logger.warning(f"[fund_fh_em] provider 返回空数据")
                return {
                    "success": True,
            ...
            unique_keys = ["基金代码", "权益登记日", "年份"]
            extra_fields = {"数据源": "akshare", "接口名称": "fund_fh_em"}
            ...
            logger.error(f"[fund_fh_em] update_single_data 发生错误: {str(e)}")
            return {
                "success": False,
                "message": str(e),
                "inserted": 0,
    ...
    async def update_batch_data(self, task_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            years = [str(y) for y in range(1999, current_year + 1)]
            
            logger.info(f"[fund_fh_em] 年份范围: {years[0]} - {years[-1]}")
            ...
            logger.info(f"[fund_fh_em] 已存在 {len(existing_years)} 个年份的数据")
            ...
            logger.info(f"[fund_fh_em] 需要更新 {total_years} 个年份")
            ...
            async def fetch_and_save(year: str):
                ...
                async with semaphore:
                    try:
                        ...
                        logger.debug(f"获取年份 {year} 分红数据失败: {e}")
                        async with lock:
                            failed += 1
                            processed += 1
            ...
            logger.info(f"[fund_fh_em] {message}")
            ...
            logger.error(f"[fund_fh_em] 批量更新失败: {e}", exc_info=True)
            if task_manager and task_id:
