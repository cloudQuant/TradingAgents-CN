"""
基金分红-东财服务（重构版：继承SimpleService，无参数接口，不需要批量更新）
"""
...
from app.services.data_sources.funds.providers.fund_fh_em_provider import FundFhEmProvider
from app.services.database.control_mongodb import ControlMongodb

logger = logging.getLogger(__name__)


class FundFhEmService(SimpleService):
    """基金分红-东财服务（无参数接口，直接获取所有数据）"""
    ...
    async def update_single_data(self, **kwargs) -> Dict[str, Any]:
        ...
        async with self._update_lock:
            try:
                ...
                # 不传递任何参数给 provider（fund_fh_em 接口不需要参数）
