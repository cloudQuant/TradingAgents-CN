"""
基金报告持股-巨潮服务（重构版：继承BaseService，需要date参数）
"""
...
from app.services.data_sources.funds.providers.fund_report_stock_cninfo_provider import FundReportStockCninfoProvider

logger = logging.getLogger(__name__)


class FundReportStockCninfoService(BaseService):
    """基金报告持股-巨潮服务（需要日期参数）"""
    
    collection_name = "fund_report_stock_cninfo"
    provider_class = FundReportStockCninfoProvider
    
    # 批量更新配置：不需要从其他集合获取代码，因为接口返回所有股票的数据
    # 但我们需要生成季度日期列表
    ...
    # 增量更新：根据日期和股票代码检查是否已存在
    incremental_check_fields = ["日期", "股票代码"]
    ...
    def _get_quarter_dates_for_select(self, start_year: int = 2017) -> List[Dict[str, str]]:
        """
        生成季度日期列表用于前端选择（格式：YYYY-MM-DD）
        
        Args:
            start_year: 起始年份，默认2017
            
        Returns:
            季度日期列表，格式：[{"value": "2017-03-31", "label": "2017-03-31"}, ...]
        """
        current_year = datetime.now().year
        quarter_dates = []
    ...
    async def _get_tasks_to_process(
        ...
        检查当前数据集合中是否存在日期和股票代码的组合，如果存在就不更新
        
        Args:
            codes: 股票代码列表（此接口不需要，因为接口返回所有股票的数据）
        ...
        # 过滤出需要处理的任务
        # 注意：existing 中的元组是 (日期, 股票代码) 格式，日期格式是 YYYY-MM-DD
        # 我们需要检查是否存在该日期的任何数据
