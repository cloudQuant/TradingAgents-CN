...
负责扫描 futures/services 目录，加载所有 Service 类，
并提供按集合名称查找服务类的功能。
"""
...
SERVICES_PACKAGE = "app.services.data_sources.futures.services"
SERVICES_PATH = Path(__file__).resolve().parent / "services"


@lru_cache()
def get_registered_futures_services() -> Dict[str, Type[BaseService]]:
    """
    获取所有已注册的 Futures Service 类
...
def get_service_class(collection_name: str) -> Optional[Type[BaseService]]:
    ...
    """
    return get_registered_futures_services().get(collection_name)


def clear_service_registry_cache():
    """清除服务注册表缓存（用于热重载）"""
    get_registered_futures_services.cache_clear()
