...
负责扫描 futures/providers 目录，加载所有 Provider 类，
并为其附加必要的元信息。
"""
...
from app.services.data_sources.futures.collection_metadata import FUTURES_COLLECTION_METADATA

# 尝试导入配置文件，如果不存在则使用空字典
try:
    from app.config.futures_update_config import FUTURES_UPDATE_CONFIGS
except ImportError:
    FUTURES_UPDATE_CONFIGS = {}

PROVIDERS_PACKAGE = "app.services.data_sources.futures.providers"
...
def _apply_metadata(provider_cls: Type[BaseProvider]):
    ...
    config = FUTURES_UPDATE_CONFIGS.get(provider_cls.collection_name, {})
    meta = FUTURES_COLLECTION_METADATA.get(provider_cls.collection_name, {})
...
@lru_cache()
def get_registered_futures_providers() -> List[Type[BaseProvider]]:
    """获取所有 Futures Provider 类"""
...
def get_provider_class(collection_name: str) -> Optional[Type[BaseProvider]]:
    """根据集合名称获取 Provider 类"""
    for provider_cls in get_registered_futures_providers():
...
def clear_provider_registry_cache():
    ...
    get_registered_futures_providers.cache_clear()
