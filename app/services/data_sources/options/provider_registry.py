...
PROVIDERS_PACKAGE = "app.services.data_sources.options.providers"


def _apply_metadata(provider_class: Type[BaseProvider]) -> None:
    """应用元信息配置到Provider类"""
    collection_name = getattr(provider_class, "collection_name", "")
    if collection_name in OPTION_COLLECTION_METADATA:
        meta = OPTION_COLLECTION_METADATA[collection_name]
...
@lru_cache()
def get_registered_option_providers() -> List[Type[BaseProvider]]:
    """
    获取所有已注册的期权数据Provider
...
def get_option_provider_by_name(collection_name: str) -> Type[BaseProvider] | None:
    """根据集合名称获取Provider类"""
    for provider_class in get_registered_option_providers():
        if provider_class.collection_name == collection_name:
            return provider_class
    return None


def get_option_collection_definitions() -> List[Dict[str, Any]]:
    """获取所有期权集合的定义信息"""
    definitions = []
    
    for provider_class in get_registered_option_providers():
        collection_name = provider_class.collection_name
        meta = OPTION_COLLECTION_METADATA.get(collection_name, {})
