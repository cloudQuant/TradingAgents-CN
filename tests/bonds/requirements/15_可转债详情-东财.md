...
### 步骤
1. 数据集合：创建一个新的数据集合，名称为**可转债详情-东财** (http://localhost:3000/bonds/collections/bond_zh_cov_info)
2. 页面功能：数据概览、详情展示、批量更新
3. 更新数据功能：
   - **单个更新**：输入可转债代码和指标类型，获取详情
   - **批量更新**：批量获取多只可转债的详情
   - **指标选择**：基本信息、中签号、筹资用途、重要日期
