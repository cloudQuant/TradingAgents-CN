"""
验证 bonds/collections 页面与需求文档 requirements 的一致性：
- 从 tests/bonds/requirements/*.md 中解析出声明的集合路由，提取集合名
- 校验 /api/bonds/collections 返回的数据集合包含这些集合名
- 访问前端页面 /bonds/collections/{name}，确认能正常打开（返回 HTTP 200）
...
def _requirements_dir() -> str:
    # 本测试文件位于 tests/bonds/collections/
...
def extract_expected_collections(requirements_dir: str) -> Dict[str, str]:
    ...
    扫描 requirements 目录的所有 .md 文档，提取形如：
      http://localhost:3000/bonds/collections/<collectionName>
    的集合路由，返回 {collectionName: 文档路径}
    """
    pattern = re.compile(r"http://localhost:3000/bonds/collections/([a-zA-Z0-9_\-]+)")
...
class TestBondsCollectionsRequirementsCoverage:
    ...
    def _extract_from_frontend_page(self, frontend_base_url: str, logger) -> Optional[list]:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                url = f"{frontend_base_url}/bonds/collections"
                ...
                if "/login" in page.url or "登录" in page.title():
                    ...
                    try:
                        ...
                        if login_button:
                            ...
                            if "/bonds/collections" not in page.url:
                ...
                # 尝试1: 查找包含 /bonds/collections/ 的链接
                links = page.query_selector_all('a[href*="/bonds/collections/"]')
                if not links:
                    links = page.query_selector_all('tr td a')
                
                for link in links:
                    href = link.get_attribute('href')
                    if href and '/bonds/collections/' in href:
                        name = href.split('/bonds/collections/')[-1].split('?')[0].split('#')[0]
                        ...
                        result = page.evaluate(
                            ...
                            async () => {
                                try {
                                    ...
                                    const resp = await fetch('/api/bonds/collections', { credentials: 'include', headers });
    ...
    def _get_collections_list(self, frontend_base_url: str, auth_headers: Dict[str, str], logger) -> Optional[list]:
        ...
        # 方式2: 尝试后端API（备选）
        backend_api_url = f"http://localhost:8000/api/bonds/collections"
    ...
    def test_requirements_collections_covered_by_api(self, frontend_base_url: str, auth_headers: Dict[str, str]):
        """
        要求：API /api/bonds/collections 返回需包含 requirements 文档中声明的集合名
        """
        # 创建日志记录器
        ...
        assert expected_map, "未在需求文档中解析到任何集合路由，请检查文档格式是否包含 /bonds/collections/{name}"
        ...
        if extra_in_api:
            ...
            for idx, name in enumerate(sorted(extra_in_api), 1):
                route = f"{frontend_base_url}/bonds/collections/{name}"
                logger.log(f"  {idx:3d}. [+] {name}")
    ...
    def test_requirements_collections_frontend_openable(
        ...
        """
        要求：需求文档中声明的集合详情页 /bonds/collections/{name} 可打开（HTTP 200）。
        若前端未启动，跳过此用例。
        """
        # 创建日志记录器
        log_file = _get_log_file_path()
        ...
        # 检查前端是否可访问
        base_ok = False
        main_url = f"{frontend_base_url}/bonds/collections"
        with httpx.Client(trust_env=False, timeout=60.0, follow_redirects=True) as client:
            try:
                r = client.get(main_url)
                base_ok = (200 <= r.status_code < 400) and r.url.path.startswith("/bonds/collections")
        ...
        with httpx.Client(trust_env=False, timeout=30.0, follow_redirects=True) as client:
            for idx, name in enumerate(target_names, 1):
                detail_url = f"{frontend_base_url}/bonds/collections/{name}"
                try:
                    r = client.get(detail_url)
                    ok = (200 <= r.status_code < 400) and r.url.path.startswith(f"/bonds/collections/{name}")
                    if not ok:
