# Bug 修复: MongoDB 无法编码 datetime.date 对象
...
从 AKShare 获取的数据中，某些字段（如`下一开放日`）的值是 Python 的`datetime.date`对象。MongoDB 的 pymongo 驱动无法直接编码`datetime.date`类型，只能编码以下类型：
...
所有从 AKShare 获取包含日期字段的基金数据保存操作都可能受此问题影响：
...
- *修改的方法**:
1. `save_fund_purchase_status_data` (第 642-662 行)
2. `save_fund_name_em_data` (第 64-84 行)
3. `save_fund_basic_info_data` (第 223-243 行)
4. `save_fund_info_index_data` (第 376-395 行)
...
# 创建包含 datetime.date 的测试数据

test_df = pd.DataFrame([{
    '基金代码': '000005',
    '基金简称': '测试基金',
    '下一开放日': date(2025, 12, 8),  # datetime.date 对象
    '申购状态': '开放申购',
    '赎回状态': '开放赎回'
}])
