...
4.**API 完整**- 后端提供完整的集合查询、分页、统计 API
...
1. **1 个真正缺失**- `fund_etf_category_sina` (优先级：中)
...
| REITs 类 | 2 | reits_realtime_em, reits_hist_em |
...
1. 📝**文档同步**- 更新需求文档中的 9 个接口名称，与后端保持一致
2. 🔍**确认需求**- 检查 `fund_etf_category_sina` 是否真的需要

### 低优先级

1. 📚**文档完善**- 为 `fund_net_value` 和 `fund_ranking` 补充需求文档
