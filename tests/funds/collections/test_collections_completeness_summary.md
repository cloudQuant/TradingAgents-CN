...
- *功能**:
- ✅ 从 requirements 文档中自动提取所有数据集合接口（70 个）
- ✅ 从后端路由文件提取已实现的数据集合（72 个）
...
- ✅ `tests/funds/test_collections_completeness.py` (278 行)
...
- ✅ `app/routers/funds.py` (2583 行，包含 72 个集合定义)
- ✅ `frontend/src/views/Funds/Collections.vue` (223 行)
- ✅ `frontend/src/api/funds.ts`
- ✅ `frontend/src/router/index.ts`
- ✅ `tests/funds/*.md` (70 个需求文档)
...
1. 📝 批量更新 9 个需求文档的接口名称
2. 🔍 与产品确认 `fund_etf_category_sina` 的必要性

### 有空时

1. 📚 为 `fund_net_value` 和 `fund_ranking` 补充需求文档
2. 🧪 编写前端 E2E 测试（Playwright）验证页面交互
3. 📊 添加数据质量监控
