"""
验证 funds/collections 页面与需求文档 requirements 的一致性：
- 从 tests/funds/*.md 中解析出声明的集合路由，提取集合名
- 校验 /api/funds/collections 返回的数据集合包含这些集合名
- 访问前端页面 /funds/collections/{name}，确认能正常打开（返回 HTTP 200）
...
def _requirements_dir() -> str:
    # 本测试文件位于 tests/funds/collections/
    # 基金的需求文档在 tests/funds/ 目录下（不是 requirements 子目录）
    here = os.path.dirname(__file__)
    req_dir = os.path.abspath(os.path.join(here, ".."))
    return req_dir
...
def extract_expected_collections(requirements_dir: str) -> Dict[str, str]:
    ...
    扫描 requirements 目录的所有 .md 文档，提取形如：
      http://localhost:3000/funds/collections/<collectionName>
    或从接口字段提取集合名称
    的集合路由，返回 {collectionName: 文档路径}
    """
    # 匹配前端路由格式
    pattern = re.compile(r"http://localhost:3000/funds/collections/([a-zA-Z0-9_\-]+)")
    # 匹配接口名称格式（基金需求文档格式）
    ...
    for root, _, files in os.walk(requirements_dir):
        for fn in files:
            ...
            try:
                ...
                # 尝试从接口格式提取
                for name in interface_pattern.findall(text):
                    mapping.setdefault(name, fp)
            except Exception:
                # 读取失败忽略该文件
                continue
...
class TestFundsCollectionsRequirementsCoverage:
    ...
    def _extract_from_frontend_page(self, frontend_base_url: str, logger) -> Optional[list]:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                url = f"{frontend_base_url}/funds/collections"
                ...
                if "/login" in page.url or "登录" in page.title():
                    ...
                    try:
                        ...
                        if login_button:
                            ...
                            # 重新访问目标页面(如果没自动跳回)
                            if "/funds/collections" not in page.url:
                ...
                # 尝试1: 查找包含 /funds/collections/ 的链接
                links = page.query_selector_all('a[href*="/funds/collections/"]')
                # 尝试1.1: 表格中的链接
                if not links:
                ...
                for link in links:
                    ...
                    if href and '/funds/collections/' in href:
                        name = href.split('/funds/collections/')[-1].split('?')[0].split('#')[0]
                ...
                # 尝试3: 在浏览器上下文内直接调用前端接口 /api/funds/collections 获取数据
                if not collections:
                    try:
                        ...
                        result = page.evaluate(
                            ...
                            async () => {
                                try {
                                    ...
                                    const resp = await fetch('/api/funds/collections', { credentials: 'include', headers });
                ...
                if not collections:
                    try:
                        logger.log(f"    ...等待页面网络响应 /api/funds/collections")
                        resp = page.wait_for_response(lambda r: '/api/funds/collections' in r.url, timeout=8000)
                        try:
    ...
    def _get_collections_list(self, frontend_base_url: str, auth_headers: Dict[str, str], logger) -> Optional[list]:
        ...
        # 方式2: 尝试后端API（备选）
        backend_api_url = f"http://localhost:8000/api/funds/collections"
    ...
    def test_requirements_collections_covered_by_api(self, frontend_base_url: str, auth_headers: Dict[str, str]):
        """
        要求：前端 API /api/funds/collections 返回需包含 requirements 文档中声明的集合名
        """
        # 创建日志记录器
        ...
        assert expected_map, "未在需求文档中解析到任何集合路由，请检查文档格式是否包含 /funds/collections/{name} 或接口: xxx"
        ...
        if extra_in_api:
            ...
            for idx, name in enumerate(sorted(extra_in_api), 1):
                route = f"{frontend_base_url}/funds/collections/{name}"
                logger.log(f"  {idx:3d}. [+] {name}")
    ...
    def test_requirements_collections_frontend_openable(
        ...
        """
        要求：需求文档中声明的集合详情页 /funds/collections/{name} 可打开（HTTP 200）。
        若前端未启动，跳过此用例。
        """
        # 创建日志记录器
        log_file = _get_log_file_path()
        ...
        if extra_in_api:
            ...
            # 逐项列出未在需求中的集合
            for idx, name in enumerate(sorted(extra_in_api), 1):
                logger.log(f"      - {idx:3d}. {name} -> {frontend_base_url}/funds/collections/{name}")
            # 紧凑清单
            logger.log(f"  额外集合名单: {', '.join(sorted(extra_in_api))}")
        ...
        # 检查前端是否可访问
        base_ok = False
        main_url = f"{frontend_base_url}/funds/collections"
        with httpx.Client(trust_env=False, timeout=60.0, follow_redirects=True) as client:
            try:
                r = client.get(main_url)
                # 要求最终URL路径仍在 /funds/collections（未被重定向到登录等）
                base_ok = (200 <= r.status_code < 400) and r.url.path.startswith("/funds/collections")
        ...
        with httpx.Client(trust_env=False, timeout=30.0, follow_redirects=True) as client:
            for idx, name in enumerate(target_names, 1):
                detail_url = f"{frontend_base_url}/funds/collections/{name}"
                try:
                    r = client.get(detail_url)
                    ok = (200 <= r.status_code < 400) and r.url.path.startswith(f"/funds/collections/{name}")
                    if not ok:
