...
- **API 接口**: `ak.fund_etf_fund_daily_em()`
...
record = {
    ...
    "date": current_date,
    "source": "akshare",              # 数据来源
    "endpoint": "fund_etf_fund_daily_em",  # API 端点
...
| 前端 UI | ✅ 自动继承 | 数据概览、列表、刷新、清空、更新 |

| 测试用例 | ✅ 已完整 | 单元测试、API 测试、E2E 测试 |
...
| updated_at | string | 更新时间戳（ISO 格式） |

### 4. 性能优化总结

#### 4.1 内存优化

- **分批处理**: 每批 500 条，避免大数据量内存溢出
...
- ✅ `test_get_fund_etf_fund_daily_stats_api`: 测试统计 API
- ✅ `test_clear_fund_etf_fund_daily_api`: 测试清空 API
