...
class TestFuturesZhSpotCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试内盘-实时行情数据数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_zh_spot()
    """
    ...
    default_params = {'market': 'CF'}
    
    def test_akshare_interface(self):
        """测试AKShare接口是否可用"""
        try:
            import akshare as ak
            result = ak.futures_zh_spot(market="CF")
            if result is None:
