"""
使用Playwright测试所有期货数据集合的前端页面加载和API更新对话框功能
"""
import os
import sys
import asyncio
...
class FrontendTester:
    ...
    async def get_all_collections(self) -> List[Dict]:
        """从后端获取所有集合列表"""
        try:
            response = await self.page.request.get(f"{BACKEND_URL}/api/futures/collections")
            if response.status == 200:
    ...
    async def test_collection_page(self, collection_name: str) -> Dict[str, Any]:
        """测试集合页面加载"""
        result = {
        ...
        try:
            # 1. 访问集合页面
            url = f"{FRONTEND_URL}/futures/collections/{collection_name}"
            ...
            if response and response.status == 200:
                ...
                # 检查是否有错误提示
                error_elements = await self.page.query_selector_all('[class*="error"], [class*="Error"]')
                if error_elements:
                    for elem in error_elements:
                        text = await elem.text_content()
                        if text and "错误" in text:
                            result["error"] = text[:100]
                            result["page_load"] = False
                            break
            else:
                result["error"] = f"页面加载失败: {response.status if response else 'No response'}"
    ...
    async def test_update_config_api(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            url = f"{BACKEND_URL}/api/futures/collections/{collection_name}/update-config"
    ...
    async def test_refresh_api(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            url = f"{BACKEND_URL}/api/futures/collections/{collection_name}/refresh"
            headers = {"Content-Type": "application/json"}
            if self.auth_token:
    ...
    try:
        ...
        for i, coll in enumerate(collections, 1):
            ...
            if refresh_ok:
                ...
                page_load_success += 1  # 假设API成功则页面也能加载
            ...
            print(f"  [{i:2d}/{len(collections)}] {status} {name}")
        ...
        with open("tests/futures/collections/frontend_test_results.json", "w", encoding="utf-8") as f:
