...
class TestFuturesShfeWarehouseReceiptCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试仓单日报-上海期货交易所数据集合
    
    接口类型：date_param
    AKShare接口：ak.futures_shfe_warehouse_receipt()
    """
    ...
    # 接口配置
    api_type = "date_param"
    akshare_func = "futures_shfe_warehouse_receipt"
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_shfe_warehouse_receipt(date="20241213")
            if result is None:
