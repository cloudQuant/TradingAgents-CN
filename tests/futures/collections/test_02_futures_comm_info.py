...
class TestFuturesCommInfoCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试期货手续费与保证金数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_comm_info()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_comm_info(symbol="所有")
            if result is None:
