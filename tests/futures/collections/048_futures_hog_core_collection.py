...
class TestFuturesHogCoreCollection:
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "核心数据" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "2024-01-01", "value": 15.5},
            {"_id": "2", "date": "2024-01-02", "value": 15.8}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({"date": ["2024-01-01"], "value": [15.5]})
        mock_ak.futures_hog_core.return_value = mock_df
        mock_db, mock_collection = MagicMock(), MagicMock()
        mock_get_db.return_value = mock_db
        mock_db.get_collection.return_value = mock_collection
        mock_collection.update_one.side_effect = lambda *args, **kwargs: MagicMock()
        response = client.post(f"{self.base_url}/update?symbol=外三元")
        assert response.status_code == 200
        assert response.json()["success"] is True
