...
class TestFuturesSettlementPriceSgxCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试新加坡交易所期货-结算价数据集合
    
    接口类型：date_param
    AKShare接口：ak.futures_settlement_price_sgx()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_settlement_price_sgx(date="20241213")
            if result is None:
