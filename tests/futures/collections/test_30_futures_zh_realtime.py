...
class TestFuturesZhRealtimeCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试内盘-实时行情数据(品种)数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_zh_realtime()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_zh_realtime(symbol="白糖")
            if result is None:
