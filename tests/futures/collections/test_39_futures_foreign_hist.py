...
class TestFuturesForeignHistCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试外盘-历史行情数据-新浪数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_foreign_hist()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_foreign_hist(symbol="GC")
            if result is None:
