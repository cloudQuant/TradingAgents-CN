...
class TestFuturesNewsShmetCollection:
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "期货资讯" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "发布时间": "2024-01-01 10:00", "内容": "铜价上涨"},
            {"_id": "2", "发布时间": "2024-01-01 11:00", "内容": "铝价下跌"}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({"发布时间": ["2024-01-01 10:00"], "内容": ["铜价上涨"]})
        mock_ak.futures_news_shmet.return_value = mock_df
        mock_db, mock_collection = MagicMock(), MagicMock()
        mock_get_db.return_value = mock_db
        mock_db.get_collection.return_value = mock_collection
        mock_collection.update_one.side_effect = lambda *args, **kwargs: MagicMock()
        response = client.post(f"{self.base_url}/update?symbol=全部")
        assert response.status_code == 200
        assert response.json()["success"] is True
