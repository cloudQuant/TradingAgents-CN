...
class TestFuturesForeignDetailCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试外盘-合约详情数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_foreign_detail()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_foreign_detail(symbol="GC")
            if result is None:
