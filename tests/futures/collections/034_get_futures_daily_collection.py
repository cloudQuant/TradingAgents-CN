...
class TestGetFuturesDailyCollection:
    """Test get_futures_daily collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "内盘-历史行情数据-交易所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "symbol": "m2401", "date": "20240101", "open": 3500.0, "close": 3520.0, "volume": 120000},
            {"_id": "2", "symbol": "m2402", "date": "20240101", "open": 3520.0, "close": 3550.0, "volume": 130000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_ak.get_futures_daily.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?start_date=20200701&end_date=20200716&market=DCE")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
