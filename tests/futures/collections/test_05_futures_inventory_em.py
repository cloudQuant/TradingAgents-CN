...
class TestFuturesInventoryEmCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试库存数据-东方财富数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_inventory_em()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_inventory_em(symbol="A")
            if result is None:
