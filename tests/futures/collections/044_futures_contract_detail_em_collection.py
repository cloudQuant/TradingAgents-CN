...
class TestFuturesContractDetailEmCollection:
    def setup_method(self):
        self.collection_name = "futures_contract_detail_em"
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "期货合约详情-东财" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "item": "合约代码", "value": "v2602F"},
            {"_id": "2", "item": "交易单位", "value": "5吨/手"}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({"item": ["合约代码"], "value": ["v2602F"]})
        mock_ak.futures_contract_detail_em.return_value = mock_df
        mock_db, mock_collection = MagicMock(), MagicMock()
        mock_get_db.return_value = mock_db
        mock_db.get_collection.return_value = mock_collection
        mock_collection.update_one.side_effect = lambda *args, **kwargs: MagicMock()
        response = client.post(f"{self.base_url}/update?symbol=v2602F")
        assert response.status_code == 200
        assert response.json()["success"] is True
