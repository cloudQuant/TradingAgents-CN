...
class TestFuturesDeliveryMatchDceCollection:
    """Test futures_delivery_match_dce collection API"""

    def setup_method(self):
        self.collection_name = "futures_delivery_match_dce"
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "交割配对-大商所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "symbol": "a", "合约号": "a2401", "配对日期": "2024-01-15", "买会员号": "001", "配对手数": 100, "卖会员号": "002"},
            {"_id": "2", "symbol": "a", "合约号": "a2401", "配对日期": "2024-01-16", "买会员号": "003", "配对手数": 200, "卖会员号": "004"}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({
            "合约号": ["a2401", "a2401"],
            "配对日期": ["2024-01-15", "2024-01-16"],
            "买会员号": ["001", "003"],
            "配对手数": [100, 200],
            "卖会员号": ["002", "004"]
        })
        mock_ak.futures_delivery_match_dce.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?symbol=a")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
