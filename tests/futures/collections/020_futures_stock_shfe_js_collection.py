...
class TestFuturesStockShfeJsCollection:
    """Test futures_stock_shfe_js collection API"""

    def setup_method(self):
        self.collection_name = "futures_stock_shfe_js"
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "上海期货交易所库存周报"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "20240419", "商品": "铜", "增减": 100, "增减幅度": 5.5},
            {"_id": "2", "date": "20240419", "商品": "铝", "增减": 200, "增减幅度": 10.5}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_ak.futures_stock_shfe_js.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?date=20240419")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
