...
class TestFuturesZhRealtimeCollection:
    """Test futures_zh_realtime collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "内盘-实时行情数据(品种)"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "symbol": "SR2401", "exchange": "CZCE", "name": "白糖2401", "trade": 5500.0, "open": 5480.0, "volume": 120000, "position": 80000},
            {"_id": "2", "symbol": "SR2405", "exchange": "CZCE", "name": "白糖2405", "trade": 5520.0, "open": 5500.0, "volume": 110000, "position": 75000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({
            "symbol": ["SR2401", "SR2405"],
            "exchange": ["CZCE", "CZCE"],
            "name": ["白糖2401", "白糖2405"],
            "trade": [5500.0, 5520.0],
            "open": [5480.0, 5500.0],
            "volume": [120000, 110000],
            "position": [80000, 75000]
        })
        mock_ak.futures_zh_realtime.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?symbol=白糖")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
