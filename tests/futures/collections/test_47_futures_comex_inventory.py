...
class TestFuturesComexInventoryCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试COMEX库存数据数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_comex_inventory()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_comex_inventory(symbol="黄金")
            if result is None:
