...
class TestFuturesDeliveryCzceCollection:
    """Test futures_delivery_czce collection API"""

    def setup_method(self):
        self.collection_name = "futures_delivery_czce"
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "交割统计-郑商所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "20210112", "品种": "白糖", "交割数量": 1000, "交割额": 5000000},
            {"_id": "2", "date": "20210112", "品种": "棉花", "交割数量": 2000, "交割额": 10000000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({
            "品种": ["白糖", "棉花"],
            "交割数量": [1000, 2000],
            "交割额": [5000000, 10000000]
        })
        mock_ak.futures_delivery_czce.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?date=20210112")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
