...
class TestFuturesSpotStockCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试现货与股票数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_spot_stock()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_spot_stock(symbol="铜")
            if result is None:
