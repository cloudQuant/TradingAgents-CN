...
class TestFuturesForeignCommodityRealtimeCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试外盘-实时行情数据数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_foreign_commodity_realtime()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_foreign_commodity_realtime(symbol="伦敦金")
            if result is None:
