...
class TestFuturesSpotSysCollection:
    """Test futures_spot_sys collection API"""
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "symbol": "铜", "indicator": "市场价格", "日期": "2024-01-01", "主力基差": 1000.5},
            {"_id": "2", "symbol": "铜", "indicator": "市场价格", "日期": "2024-01-02", "主力基差": 1020.3}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_ak.futures_spot_sys.return_value = mock_df
        ...
        # This collection needs symbol and indicator parameters
        response = client.post(f"{self.base_url}/update?symbol=铜&indicator=市场价格")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
