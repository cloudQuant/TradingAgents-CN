...
class TestFuturesZhDailySinaCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试内盘-历史行情数据-新浪数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_zh_daily_sina()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_zh_daily_sina(symbol="RB0")
            if result is None:
