...
class TestFuturesToSpotDceCollection:
    """Test futures_to_spot_dce collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "期转现-大商所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "202312", "合约代码": "m2401", "期转现发生日期": "2023-12-01", "期转现数量": 100},
            {"_id": "2", "date": "202312", "合约代码": "m2401", "期转现发生日期": "2023-12-02", "期转现数量": 200}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({
            "合约代码": ["m2401", "m2401"],
            "期转现发生日期": ["2023-12-01", "2023-12-02"],
            "期转现数量": [100, 200]
        })
        mock_ak.futures_to_spot_dce.return_value = mock_df
        ...
        # The update for this one needs query param 'date' (year-month format)
