...
class TestFuturesHoldPosSinaCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试成交持仓-新浪数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_hold_pos_sina()
    """
    ...
    default_params = {'symbol': 'IF'}
    
    def test_akshare_interface(self):
        """测试AKShare接口是否可用"""
        try:
            import akshare as ak
            result = ak.futures_hold_pos_sina(symbol="成交量", contract="RB2501", date="20241213")
            if result is None:
