...
class TestFuturesWarehouseReceiptDceCollection:
    """Test futures_warehouse_receipt_dce collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "仓单日报-大连商品交易所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "20251027", "symbol": "a", "warehouse": "WH1", "receipt": 1000},
            {"_id": "2", "date": "20251027", "symbol": "a", "warehouse": "WH2", "receipt": 2000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        # ak.futures_warehouse_receipt_dce returns a dict of DataFrames
        mock_df = pd.DataFrame({
            "仓库/分库": ["WH1", "WH2"],
            "今日仓单量（手）": [1000, 2000]
        })
        mock_ak.futures_warehouse_receipt_dce.return_value = {"a": mock_df}
        ...
        response = client.post(f"{self.base_url}/update?date=20251027")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
