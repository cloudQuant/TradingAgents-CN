...
class TestFuturesMainSinaCollection:
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "期货连续合约" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "日期": "2024-01-01", "开盘价": 4000, "收盘价": 4020, "成交量": 100000},
            {"_id": "2", "日期": "2024-01-02", "开盘价": 4020, "收盘价": 4040, "成交量": 110000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({"日期": ["2024-01-01"], "开盘价": [4000], "收盘价": [4020]})
        mock_ak.futures_main_sina.return_value = mock_df
        mock_db, mock_collection = MagicMock(), MagicMock()
        mock_get_db.return_value = mock_db
        mock_db.get_collection.return_value = mock_collection
        mock_collection.update_one.side_effect = lambda *args, **kwargs: MagicMock()
        response = client.post(f"{self.base_url}/update?symbol=IF0&start_date=19900101&end_date=22220101")
        assert response.status_code == 200
        assert response.json()["success"] is True
