...
class TestFuturesMainSinaCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试期货连续合约-新浪数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_main_sina()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_main_sina(symbol="V0")
            if result is None:
