...
class TestFuturesIndexCcidxCollection:
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "中证商品指数" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "日期": "2024-01-01", "指数代码": "000001", "收盘点位": 1000.5, "涨跌幅": 1.2},
            {"_id": "2", "日期": "2024-01-02", "指数代码": "000001", "收盘点位": 1012.0, "涨跌幅": 1.15}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({"日期": ["2024-01-01"], "指数代码": ["000001"], "收盘点位": [1000.5]})
        mock_ak.futures_index_ccidx.return_value = mock_df
        mock_db, mock_collection = MagicMock(), MagicMock()
        mock_get_db.return_value = mock_db
        mock_db.get_collection.return_value = mock_collection
        mock_collection.update_one.side_effect = lambda *args, **kwargs: MagicMock()
        response = client.post(f"{self.base_url}/update?symbol=中证商品期货指数")
        assert response.status_code == 200
        assert response.json()["success"] is True
