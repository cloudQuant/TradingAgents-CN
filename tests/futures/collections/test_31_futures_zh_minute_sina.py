...
class TestFuturesZhMinuteSinaCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试内盘-分时行情数据数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_zh_minute_sina()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_zh_minute_sina(symbol="IF2501", period="1")
            if result is None:
