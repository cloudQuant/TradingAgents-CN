...
class TestFuturesContractInfoIneCollection:
    """Test futures_contract_info_ine collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "上海国际能源交易中心"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "合约代码": "sc2401", "上市日": "20231015", "到期日": "20240115", "开始交割日": "20240116", "最后交割日": "20240119", "挂牌基准价": 520.0, "交易日": "20241129"},
            {"_id": "2", "合约代码": "sc2402", "上市日": "20231115", "到期日": "20240215", "开始交割日": "20240216", "最后交割日": "20240219", "挂牌基准价": 525.0, "交易日": "20241129"}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_df = pd.DataFrame({
            ...
            "挂牌基准价": [520.0, 525.0],
            "交易日": ["20241129", "20241129"]
        })
        mock_ak.futures_contract_info_ine.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?date=20241129")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
