...
class TestFuturesShfeWarehouseReceiptCollection:
    """Test futures_shfe_warehouse_receipt collection API"""
    ...
    def test_collection_info_exists(self):
        ...
        for col in collections:
            if col["name"] == self.collection_name:
                found = True
                assert col["display_name"] == "仓单日报-上海期货交易所"
                break
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "date": "20200702", "symbol": "CU", "warehouse": "WH1", "receipt": 1000},
            {"_id": "2", "date": "20200702", "symbol": "CU", "warehouse": "WH2", "receipt": 2000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        # ak.futures_shfe_warehouse_receipt returns a dict of DataFrames
        mock_df = pd.DataFrame({
            "仓库": ["WH1", "WH2"],
            "期货": [1000, 2000]
        })
        mock_ak.futures_shfe_warehouse_receipt.return_value = {"CU": mock_df}
        ...
        response = client.post(f"{self.base_url}/update?date=20200702")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
