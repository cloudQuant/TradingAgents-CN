...
class TestFuturesForeignCommodityRealtimeCollection:
    ...
    def test_collection_info_exists(self):
        ...
        found = any(col["name"] == self.collection_name and col["display_name"] == "外盘-实时行情数据" 
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "名称": "COMEX黄金", "最新价": 1800.5, "涨跌幅": 0.5, "成交量": 100000},
            {"_id": "2", "名称": "NYMEX原油", "最新价": 75.3, "涨跌幅": -0.3, "成交量": 200000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_ak.futures_foreign_commodity_realtime.return_value = mock_df
        ...
        response = client.post(f"{self.base_url}/update?symbol=GC")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
