...
class TestFuturesWarehouseReceiptDceCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试仓单日报-大连商品交易所数据集合
    
    接口类型：date_param
    AKShare接口：ak.futures_warehouse_receipt_dce()
    """
    ...
    # 接口配置
    api_type = "date_param"
    akshare_func = "futures_warehouse_receipt_dce"
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_warehouse_receipt_dce(date="20241213")
            if result is None:
