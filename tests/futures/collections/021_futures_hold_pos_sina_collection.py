...
class TestFuturesHoldPosSinaCollection:
    """Test futures_hold_pos_sina collection API"""

    def setup_method(self):
        self.collection_name = "futures_hold_pos_sina"
    ...
    def test_get_data(self, mock_get_db):
        ...
        items = [
            {"_id": "1", "symbol": "成交量", "contract": "OI2501", "date": "20240223", "名次": 1, "会员简称": "会员A", "成交量": 1000},
            {"_id": "2", "symbol": "成交量", "contract": "OI2501", "date": "20240223", "名次": 2, "会员简称": "会员B", "成交量": 2000}
        ]
    ...
    def test_update_data(self, mock_get_db):
        ...
        mock_ak.futures_hold_pos_sina.return_value = mock_df
        ...
        # Note: This collection needs symbol parameter with special format
        response = client.post(f"{self.base_url}/update?symbol=成交量:OI2501:20240223")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
