...
class TestFuturesGlobalHistEmCollection(FuturesCollectionTestBase, PlaywrightAPITestMixin):
    """测试外盘-历史行情数据-东财数据集合
    
    接口类型：symbol_param
    AKShare接口：ak.futures_global_hist_em()
    """
    ...
    def test_akshare_interface(self):
        ...
        try:
            ...
            result = ak.futures_global_hist_em(symbol="伦敦金")
            if result is None:
