...
参考基金数据集合的实现，为股票数据集合创建了完全定制化的 API 更新对话框。

- *核心特性**：
- **配置驱动**：通过`collectionRefreshConfig.ts`配置每个集合的 UI 和参数
- **多种 UI 类型**：支持`none`（无参数）、`single`（单个更新）、`batch`（批量更新）、`single-batch`（单个+批量）
...
1. ✅ **stock_individual_info_em**（个股信息查询-东财）
   - UI 类型：`single-batch`
   - 单个更新：支持输入股票代码
   - 批量更新：从沪深京 A 股获取代码列表，支持并发和延迟配置

1. ✅ **stock_individual_basic_info_xq**（个股信息查询-雪球）
   - UI 类型：`single-batch`
   - 单个更新：支持雪球格式代码（SH/SZ 前缀）
   - 批量更新：支持并发和延迟配置

1. ✅ **stock_zh_a_hist**（A 股历史行情-东财）
   - UI 类型：`single-batch`
   - 单个更新：股票代码 + 周期选择 + 复权类型选择
   - 批量更新：周期 + 复权类型 + 并发配置

1. ✅ **stock_zh_a_hist_min_em**（A 股分时数据-东财）
   - UI 类型：`single`
   - 单个更新：股票代码 + 分钟周期选择 + 复权类型

1. ✅ **stock_zh_a_spot_em**（沪深京 A 股实时行情-东财）
   - UI 类型：`none`
   - 全部更新：一次性获取所有 A 股实时行情

### 3. Collection.vue UI 实现
...
- ✅ **string**：文本输入框，支持 placeholder 和 description
- ✅ **select**：下拉选择框，从配置的 options 中选择
- ✅ **number**：数字输入框，支持 min/max/step 配置
- ⏳ **date**：日期选择器（待实现）
- ⏳ **boolean**：开关（待实现）
...
## 📊 对比：JSON 输入 vs 定制化 UI

| 特性 | JSON 输入（旧） | 定制化 UI（新） |

|------|--------------|--------------|

| 用户友好度 | ⭐⭐ 需要了解 JSON 格式 | ⭐⭐⭐⭐⭐ 所见即所得 |
...
3. 设置请求延迟：`0.5`秒
4. 点击"批量更新"按钮
5. 系统从沪深京 A 股实时行情中获取所有股票代码，批量更新
...
当前存在 2 个可忽略的 style 类型警告：

- 位置：template 中的`<el-table style="width: 100%">`
- 原因：Element Plus 实际支持 string 类型的 style，但类型定义要求 CSSProperties
- 影响：无，可正常运行
...
- `tests/stocks/requirements/README_API_UPDATE.md` - API 更新总文档
