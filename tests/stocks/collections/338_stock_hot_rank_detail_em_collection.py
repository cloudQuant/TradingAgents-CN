"""
A股股票热度-历史趋势及粉丝特征-数据集合测试

测试目标：
1. 验证 stock_hot_rank_detail_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hot_rank_detail_em
- 描述：东方财富网-股票热度-历史趋势及粉丝特征-A股
- 输入参数：symbol (默认 "SZ000665")
- 输出参数：时间, 排名, 证券代码, 新晋粉丝, 铁杆粉丝
"""
...
class TestStockHotRankDetailEmCollection:
    """A股股票热度-历史趋势及粉丝特征 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "stock_hot_rank_detail_em"
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "时间" in item or "time" in item, "应该包含时间字段"
            assert "排名" in item or "rank" in item, "应该包含排名字段"
            assert "证券代码" in item or "code" in item, "应该包含证券代码字段"
            assert "新晋粉丝" in item or "new_fans" in item, "应该包含新晋粉丝字段"
            assert "铁杆粉丝" in item or "loyal_fans" in item, "应该包含铁杆粉丝字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_rank_detail_em"
