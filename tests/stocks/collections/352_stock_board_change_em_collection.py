...
测试目标：
1. 验证 stock_board_change_em 数据集合的完整功能
...
- 输入参数：无
- 输出参数：板块, 涨跌幅, 主力净流入, 板块异动总次数, 板块异动最频繁个股及所属类型-股票代码, 板块异动最频繁个股及所属类型-买卖方向, 板块具体异动类型列表及出现次数
"""
...
class TestStockBoardChangeEmCollection:
    """板块异动详情 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert any(key in item for key in ["板块", "board", "板块名称"]), "应该包含板块字段"
            assert any(key in item for key in ["涨跌幅", "change"]), "应该包含涨跌幅字段"
            assert any(key in item for key in ["主力净流入", "net_inflow"]), "应该包含主力净流入字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_board_change_em"
