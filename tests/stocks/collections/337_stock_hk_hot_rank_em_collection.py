...
测试目标：
1. 验证 stock_hk_hot_rank_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hk_hot_rank_em
- 描述：东方财富-个股人气榜-人气榜-港股市场
- 输入参数：无
- 输出参数：当前排名, 代码, 最新价, 涨跌幅
"""
...
class TestStockHkHotRankEmCollection:
    """港股人气榜 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "当前排名" in item or "rank" in item, "应该包含当前排名字段"
            assert "代码" in item or "code" in item, "应该包含代码字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hk_hot_rank_em"
