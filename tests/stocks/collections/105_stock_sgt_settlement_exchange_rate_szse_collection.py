...
async def test_collection_endpoint_exists():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse",
...
async def test_collection_fields():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse?page=1&page_size=10",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
        if response.status_code == 200:
            data = response.json()
            if data["data"]:
                item = data["data"][0]
                expected_fields = ["适用日期", "买入结算汇兑比率", "卖出结算汇兑比率", "货币种类"]
...
async def test_refresh_collection():
    ...
    async with AsyncClient(base_url=BASE_URL, timeout=300.0) as client:
        response = await client.post(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse/refresh",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"},
            json={}
        )
...
async def test_collection_overview():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse/overview",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
...
async def test_collection_pagination():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse?page=1&page_size=5",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
        if response.status_code == 200:
            data = response.json()
...
async def test_clear_collection():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.delete(
            "/stocks/collections/stock_sgt_settlement_exchange_rate_szse/clear",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
        assert response.status_code == 200
...
def test_ui_update_data_flow():
    ...
    collection_url = f"{frontend_url}/stocks/collections/stock_sgt_settlement_exchange_rate_szse"
