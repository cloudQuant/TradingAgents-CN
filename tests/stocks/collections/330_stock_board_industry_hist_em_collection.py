...
测试目标：
1. 验证 stock_board_industry_hist_em 数据集合的完整功能
...
- 输入参数：symbol (默认 "小金属"), start_date (默认 "20211201"), end_date (默认 "20220401"), period (默认 "日k"), adjust (默认 "")
- 输出参数：日期, 开盘, 收盘, 最高, 最低, 涨跌幅, 涨跌额, 成交量, 成交额, 振幅, 换手率
"""
...
class TestStockBoardIndustryHistEmCollection:
    """东方财富-指数-日频 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "日期" in item or "date" in item, "应该包含日期字段"
            assert "开盘" in item or "open" in item, "应该包含开盘字段"
            assert "收盘" in item or "close" in item, "应该包含收盘字段"
            assert "最高" in item or "high" in item, "应该包含最高字段"
            assert "最低" in item or "low" in item, "应该包含最低字段"
            assert "涨跌幅" in item or "change_pct" in item, "应该包含涨跌幅字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_board_industry_hist_em"
