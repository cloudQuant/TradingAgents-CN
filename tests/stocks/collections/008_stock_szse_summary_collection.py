"""
深圳证券交易所-市场总貌-证券类别统计 数据集合测试

测试目标：
1. 验证 stock_szse_summary 数据集合的完整功能
...
- 输入参数：date (日期，格式如 "20200619")
- 输出参数：证券类别、数量、成交金额、总市值、流通市值
"""
...
class TestStockSZSESummaryCollection:
    """深圳证券交易所-证券类别统计 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "证券类别" in item or "category" in item, "应该包含证券类别字段"
            # 可以包含其他字段：数量、成交金额、总市值、流通市值
    ...
    def test_refresh_endpoint_exists(self, api_base_url, auth_headers, collection_name):
        ...
        with httpx.Client(trust_env=False, timeout=30.0) as client:
            response = client.post(url, headers=auth_headers, json={"date": yesterday})
    ...
    def test_refresh_creates_data(self, api_base_url, auth_headers, collection_name):
        ...
        with httpx.Client(trust_env=False, timeout=60.0) as client:
            refresh_response = client.post(refresh_url, headers=auth_headers, json={"date": yesterday})
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_szse_summary"
