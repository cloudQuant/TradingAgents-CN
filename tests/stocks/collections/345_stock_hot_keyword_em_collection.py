...
测试目标：
1. 验证 stock_hot_keyword_em 数据集合的完整功能
...
class TestStockHotKeywordEmCollection:
    """热门关键词 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert "概念代码" in item or "concept_code" in item, "应该包含概念代码字段"
            assert "关键词" in item or "keyword" in item, "应该包含关键词字段"
            assert "热度" in item or "hot" in item, "应该包含热度字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_keyword_em"
