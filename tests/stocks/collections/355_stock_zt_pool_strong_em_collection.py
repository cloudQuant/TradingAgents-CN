...
测试目标：
1. 验证 stock_zt_pool_strong_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_zt_pool_strong_em
- 描述：东方财富网-行情中心-涨停板行情-强势股池
- 输入参数：date (默认当前日期)
- 输出参数：序号、代码、涨跌幅、最新价、涨停价、成交额、流通市值、总市值、换手率、涨速、是否新高、量比、涨停统计、入选理由、所属行业
"""
...
class TestStockZtPoolStrongEmCollection:
    """强势股池 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["入选理由", "reason"]), "应该包含入选理由字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_zt_pool_strong_em"
