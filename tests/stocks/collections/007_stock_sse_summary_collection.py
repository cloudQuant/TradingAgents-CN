"""
上海证券交易所-股票数据总貌 数据集合测试

测试目标：
1. 验证 stock_sse_summary 数据集合的完整功能
...
class TestStockSSESummaryCollection:
    """上海证券交易所-股票数据总貌 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在（根据接口输出参数）
            assert "项目" in item or "item" in item, "应该包含项目字段"
            # 可以包含其他字段：股票、科创板、主板
    ...
    def test_data_validity(self, api_base_url, auth_headers, collection_name):
        ...
        if data["data"]["items"]:
            ...
            # 数据应该包含时间戳或更新时间
            assert any(key in item for key in ["trade_date", "date", "updated_at", "timestamp"]), \
                "数据应该包含时间信息"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_sse_summary"
