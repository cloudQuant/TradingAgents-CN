...
测试目标：
1. 验证 stock_zt_pool_previous_em 数据集合的完整功能
...
- 输入参数：date (默认当前日期)
- 输出参数：序号、代码、涨跌幅、最新价、涨停价、成交额、流通市值、总市值、换手率、涨速、振幅、昨日封板时间、昨日连板数、涨停统计、所属行业
"""
...
class TestStockZtPoolPreviousEmCollection:
    """昨日涨停股池 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "stock_zt_pool_previous_em"
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["昨日连板数", "last_limit_up_count"]), "应该包含昨日连板数字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_zt_pool_previous_em"
