...
测试目标：
1. 验证 stock_esg_hz_sina 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_esg_hz_sina
- 描述：新浪财经-ESG评级中心-ESG评级-华证指数
- 输入参数：无
- 输出参数：日期、股票代码、ESG评分、ESG等级等
"""
...
class TestStockEsgHzSinaCollection:
    """华证指数-ESG评级 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["ESG等级", "esg_grade"]), "应该包含ESG等级字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_esg_hz_sina"
