...
测试目标：
1. 验证 stock_board_industry_hist_min_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_board_industry_hist_min_em
- 描述：东方财富-沪深板块-行业板块-分时历史行情数据
- 输入参数：symbol (默认 "小金属"), period (默认 "1")
- 输出参数：日期时间, 开盘, 收盘, 最高, 最低, 成交量, 成交额, 涨跌幅, 涨跌额, 振幅, 换手率
"""
...
class TestStockBoardIndustryHistMinEmCollection:
    """东方财富-指数-分时 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "日期时间" in item or "date_time" in item, "应该包含日期时间字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_board_industry_hist_min_em"
