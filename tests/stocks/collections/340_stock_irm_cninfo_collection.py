...
测试目标：
1. 验证 stock_irm_cninfo 数据集合的完整功能
...
- 描述：互动易-提问
- 输入参数：symbol (默认 "002594")
- 输出参数：股票代码, 公司简称, 行业, 行业代码, 问题, 提问者, 来源, 提问时间, 更新时间, 提问者编号, 问题编号, 回答ID, 回答内容, 回答者
"""
...
class TestStockIrmCninfoCollection:
    """互动易-提问 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert "问题编号" in item or "question_id" in item, "应该包含问题编号字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_irm_cninfo"
