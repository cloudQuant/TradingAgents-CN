...
- 前端 JavaScript 会调用后端 API 获取数据

- *后端（FastAPI）**
- API 端点：`<http://localhost:8000/api/stocks/collections`> ← 返回 JSON 数据
...
移除了错误的"前端 API 代理"检查，现在只检查 2 种正确的方式：

1. ✅ 后端 API 直连
2. ✅ Playwright 页面 DOM 提取
...
```bash
测试 → <http://localhost:8000/api/stocks/collections> → 获取 JSON 数据
     ↓
   ✅ 成功！91 个集合

```bash

### 方式 2: Playwright DOM 提取（备选）

```bash
测试 → 启动浏览器
     ↓
   访问 <http://localhost:3000/stocks/collections（HTML> 页面）
     ↓
   等待前端 JavaScript 加载数据
     ↓
   从 DOM 中提取集合链接
     ↓
   ✅ 成功！91 个集合
...
测试策略：
方式 1: 直接访问后端 API（最快，最优先）
方式 2: 启动浏览器模拟用户访问前端页面（最可靠，备选）
