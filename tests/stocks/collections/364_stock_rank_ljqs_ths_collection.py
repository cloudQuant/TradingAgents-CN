...
测试目标：
1. 验证 stock_rank_ljqs_ths 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_rank_ljqs_ths
- 描述：同花顺-数据中心-技术选股-量价齐升
- 输入参数：无
- 输出参数：序号、股票代码、股票简称、最新价、量价齐升天数、阶段涨幅、累计换手率、所属行业
"""
...
class TestStockRankLjqsThsCollection:
    """量价齐升 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["量价齐升天数", "days"]), "应该包含量价齐升天数字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_rank_ljqs_ths"
