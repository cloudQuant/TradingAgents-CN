...
测试目标：
1. 验证 stock_hot_rank_detail_realtime_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hot_rank_detail_realtime_em
- 描述：东方财富网-个股人气榜-实时变动-A股
...
class TestStockHotRankDetailRealtimeEmCollection:
    """A股个股人气榜-实时变动 集合测试类"""
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_rank_detail_realtime_em"
