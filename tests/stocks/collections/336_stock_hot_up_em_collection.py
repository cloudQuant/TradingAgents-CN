...
测试目标：
1. 验证 stock_hot_up_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hot_up_em
- 描述：东方财富-个股人气榜-飙升榜
- 输入参数：无
- 输出参数：排名较昨日变动, 当前排名, 代码, 最新价, 涨跌额, 涨跌幅
"""
...
class TestStockHotUpEmCollection:
    """飙升榜-A股 集合测试类"""
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_up_em"
