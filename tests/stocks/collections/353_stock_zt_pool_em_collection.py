...
测试目标：
1. 验证 stock_zt_pool_em 数据集合的完整功能
...
- 输入参数：date (默认当前日期)
- 输出参数：序号、代码、涨跌幅、最新价、成交额、流通市值、总市值、换手率、封板资金、首次封板时间、最后封板时间、炸板次数、涨停统计、连板数、所属行业
"""
...
class TestStockZtPoolEmCollection:
    """涨停股池 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["连板数", "limit_up_count"]), "应该包含连板数字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_zt_pool_em"
