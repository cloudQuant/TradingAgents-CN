...
测试目标：
1. 验证 stock_hk_hot_rank_latest_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hk_hot_rank_latest_em
- 描述：东方财富-个股人气榜-最新排名-港股
...
class TestStockHkHotRankLatestEmCollection:
    """港股个股人气榜-最新排名 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "item" in item, "应该包含 item 字段"
            assert "value" in item, "应该包含 value 字段"
            assert "code" in item, "应该包含 code 字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hk_hot_rank_latest_em"
