...
测试目标：
1. 验证 stock_esg_rate_sina 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_esg_rate_sina
- 描述：新浪财经-ESG评级中心-ESG评级-ESG评级数据
- 输入参数：无
- 输出参数：成分股代码、评级机构、评级、评级季度、标识、交易市场
"""
...
class TestStockEsgRateSinaCollection:
    """ESG评级数据 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert any(key in item for key in ["成分股代码", "symbol", "code"]), "应该包含成分股代码字段"
            assert any(key in item for key in ["评级机构", "agency"]), "应该包含评级机构字段"
            assert any(key in item for key in ["评级", "grade"]), "应该包含评级字段"
            assert any(key in item for key in ["评级季度", "quarter"]), "应该包含评级季度字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_esg_rate_sina"
