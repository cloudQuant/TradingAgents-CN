...
测试目标：
1. 验证 stock_cg_equity_mortgage_cninfo 数据集合的完整功能
...
async def test_collection_endpoint_exists():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_cg_equity_mortgage_cninfo",
...
async def test_refresh_collection():
    ...
    async with AsyncClient(base_url=BASE_URL, timeout=300.0) as client:
        response = await client.post(
            "/stocks/collections/stock_cg_equity_mortgage_cninfo/refresh",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"},
            json={}
        )
...
async def test_collection_overview():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/stocks/collections/stock_cg_equity_mortgage_cninfo/overview",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
...
async def test_clear_collection():
    ...
    async with AsyncClient(base_url=BASE_URL) as client:
        response = await client.delete(
            "/stocks/collections/stock_cg_equity_mortgage_cninfo/clear",
            headers={"Authorization": f"Bearer {AUTH_TOKEN}"}
        )
        assert response.status_code == 200
...
def test_ui_update_data_flow():
    ...
    collection_url = f"{frontend_url}/stocks/collections/stock_cg_equity_mortgage_cninfo"
