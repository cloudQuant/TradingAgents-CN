...
测试目标：
1. 验证 stock_inner_trade_xq 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_inner_trade_xq
- 描述：雪球-行情中心-沪深股市-内部交易
- 输入参数：无
- 输出参数：股票代码, 变动日期, 变动人, 变动股数, 成交均价, 变动后持股数, 与董监高关系, 董监高职务
"""
...
class TestStockInnerTradeXqCollection:
    """内部交易 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert "变动日期" in item, "应该包含变动日期字段"
            assert "变动人" in item, "应该包含变动人字段"
            assert "变动股数" in item, "应该包含变动股数字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_inner_trade_xq"
