...
测试目标：
1. 验证 stock_board_industry_cons_em 数据集合的完整功能
...
- 输入参数：symbol (默认 "小金属")
- 输出参数：序号, 代码, 最新价, 涨跌幅, 涨跌额, 成交量, 成交额, 振幅, 最高, 最低, 今开, 昨收, 换手率, 市盈率-动态, 市净率
"""
...
class TestStockBoardIndustryConsEmCollection:
    """东方财富-成份股 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "序号" in item or "order" in item, "应该包含序号字段"
            ...
            assert "成交量" in item or "volume" in item, "应该包含成交量字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_board_industry_cons_em"
