"""
雪球财经-个股-公司概况-公司简介 数据集合测试

测试目标：
1. 验证 stock_individual_basic_info_xq 数据集合的完整功能
...
- 输入参数：symbol (股票代码，如"SH601127"), token, timeout
- 输出参数：item, value 等
"""
...
class TestStockIndividualBasicInfoXQCollection:
    """雪球-个股基础信息 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert any(key in item for key in ["symbol", "code", "stock_code"]), "应该包含股票代码字段"
            assert any(key in item for key in ["item", "value", "info"]), "应该包含信息字段"
    
    # ========== 测试2：数据刷新测试 ==========
    
    def test_refresh_single_stock(self, api_base_url, auth_headers, collection_name, test_symbol):
        """测试2.1：验证单个股票刷新"""
        if "Authorization" not in auth_headers:
            pytest.skip("未提供 TEST_AUTH_TOKEN，跳过刷新测试")
        ...
        with httpx.Client(trust_env=False, timeout=30.0) as client:
            response = client.post(url, headers=auth_headers, json={"symbol": test_symbol})
        
        # 刷新接口应该存在
        assert response.status_code in [200, 202, 400, 500], \
            f"刷新接口应该存在，实际状态码: {response.status_code}"
    
    def test_refresh_creates_data(self, api_base_url, auth_headers, collection_name, test_symbol):
        """测试2.2：验证刷新功能能够创建数据"""
        if "Authorization" not in auth_headers:
        ...
        with httpx.Client(trust_env=False, timeout=60.0) as client:
            refresh_response = client.post(refresh_url, headers=auth_headers, json={"symbol": test_symbol})
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_individual_basic_info_xq"
