...
测试目标：
1. 验证 stock_esg_zd_sina 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_esg_zd_sina
- 描述：新浪财经-ESG评级中心-ESG评级-秩鼎
- 输入参数：无
- 输出参数：股票代码、ESG评分、环境总评、社会责任总评、治理总评、评分日期
"""
...
class TestStockEsgZdSinaCollection:
    """秩鼎-ESG评级 集合测试类"""
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_esg_zd_sina"
