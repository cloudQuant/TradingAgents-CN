...
测试目标：
1. 验证 stock_hot_rank_relate_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hot_rank_relate_em
- 描述：东方财富-个股人气榜-相关股票
- 输入参数：symbol (默认 "SZ000665")
- 输出参数：时间、股票代码、相关股票代码、涨跌幅
"""
...
class TestStockHotRankRelateEmCollection:
    """相关股票 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "stock_hot_rank_relate_em"
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert any(key in item for key in ["时间", "time"]), "应该包含时间字段"
            assert any(key in item for key in ["股票代码", "code"]), "应该包含股票代码字段"
            assert any(key in item for key in ["相关股票代码", "related_code"]), "应该包含相关股票代码字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_rank_relate_em"
