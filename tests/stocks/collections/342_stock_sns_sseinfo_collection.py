...
测试目标：
1. 验证 stock_sns_sseinfo 数据集合的完整功能
...
- 描述：上证e互动-提问与回答
- 输入参数：symbol (默认 "603119")
- 输出参数：股票代码, 公司简称, 问题, 回答, 问题时间, 回答时间, 问题来源, 回答来源, 用户名
"""
...
class TestStockSnsSseinfoCollection:
    """上证e互动 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert "回答" in item or "answer" in item, "应该包含回答字段"
            assert "问题时间" in item or "question_time" in item, "应该包含问题时间字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_sns_sseinfo"
