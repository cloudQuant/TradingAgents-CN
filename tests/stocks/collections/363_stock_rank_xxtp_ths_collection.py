...
测试目标：
1. 验证 stock_rank_xxtp_ths 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_rank_xxtp_ths
- 描述：同花顺-数据中心-技术选股-向下突破
- 输入参数：symbol (默认 500日均线)
- 输出参数：序号、股票代码、股票简称、最新价、成交额、成交量、涨跌幅、换手率
"""
...
class TestStockRankXxtpThsCollection:
    """向下突破 集合测试类"""
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_rank_xxtp_ths"
