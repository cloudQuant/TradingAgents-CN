...
测试目标：
1. 验证 stock_hot_search_baidu 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_hot_search_baidu
- 描述：百度股市通-热搜股票
- 输入参数：symbol (默认 "A股"), date, time
- 输出参数：排名, 代码, 名称, 现价, 涨跌幅, 综合热度
"""
...
class TestStockHotSearchBaiduCollection:
    """热搜股票 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["现价", "price", "最新价"]), "应该包含现价字段"
            assert any(key in item for key in ["涨跌幅", "change"]), "应该包含涨跌幅字段"
            assert any(key in item for key in ["综合热度", "heat", "score"]), "应该包含综合热度字段"
    ...
    def test_refresh_creates_data(self, api_base_url, auth_headers, collection_name):
        ...
        params = {"symbol": "A股", "date": today, "time": "今日"}
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_hot_search_baidu"
