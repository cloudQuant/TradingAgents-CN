...
测试目标：
1. 验证 stock_changes_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_changes_em
- 描述：东方财富-行情中心-盘口异动数据
- 输入参数：symbol (默认 "大笔买入")
- 输出参数：时间、代码、名称、板块、相关信息
"""
...
class TestStockChangesEmCollection:
    """盘口异动 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["相关信息", "info"]), "应该包含相关信息字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_changes_em"
