"""
深圳证券交易所-统计资料-股票行业成交数据 数据集合测试

测试目标：
1. 验证 stock_szse_sector_summary 数据集合的完整功能
...
- 输入参数：symbol (choice of {"当月", "当年"}), date (年月，格式如 "202501")
- 输出参数：交易天数、成交金额、成交股数、成交笔数等
"""
...
class TestStockSZSESectorSummaryCollection:
    """深圳证券交易所-股票行业成交数据 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在（字段名可能是中文或英文）
            assert any(key in item for key in ["交易天数", "trading_days"]), "应该包含交易天数字段"
            assert any(key in item for key in ["成交金额-人民币元", "amount", "成交金额"]), "应该包含成交金额字段"
    ...
    def test_refresh_endpoint_exists(self, api_base_url, auth_headers, collection_name):
        ...
        now = datetime.now()
        if now.month == 1:
            date_str = f"{now.year - 1}12"
        else:
            date_str = f"{now.year}{now.month - 1:02d}"
    ...
    def test_refresh_creates_data(self, api_base_url, auth_headers, collection_name):
        ...
        with httpx.Client(trust_env=False, timeout=60.0) as client:
            refresh_response = client.post(refresh_url, headers=auth_headers, json={
                "symbol": "当月",
                "date": date_str
            })
    ...
    def test_data_validity(self, api_base_url, auth_headers, collection_name):
        """测试5.1：验证数据有效性"""
        if "Authorization" not in auth_headers:
            pytest.skip("未提供 TEST_AUTH_TOKEN，跳过数据验证")
        
        url = f"{api_base_url}/api/stocks/collections/{collection_name}/data"
        
        with httpx.Client(trust_env=False, timeout=10.0) as client:
            response = client.get(url, headers=auth_headers, params={"page": 1, "page_size": 1})
        ...
        if data["data"]["items"]:
            ...
            # 数据应该包含日期或时间戳
            assert any(key in item for key in ["date", "symbol", "updated_at", "timestamp"]), \
                "数据应该包含时间或标识信息"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_szse_sector_summary"
