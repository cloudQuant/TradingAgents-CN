"""
深圳证券交易所-市场总貌-地区交易排序 数据集合测试

测试目标：
1. 验证 stock_szse_area_summary 数据集合的完整功能
...
- 输入参数：date (年月，格式如 "202203")
- 输出参数：序号、地区、总交易额、占市场、股票交易额、基金交易额、债券交易额等
"""
...
class TestStockSZSEAreaSummaryCollection:
    """深圳证券交易所-地区交易排序 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert "地区" in item or "area" in item, "应该包含地区字段"
            # 可以包含其他字段：总交易额、占市场等
    ...
    def test_refresh_endpoint_exists(self, api_base_url, auth_headers, collection_name):
        ...
        if last_month.month == 1:
            date_str = f"{last_month.year - 1}12"
        else:
            date_str = f"{last_month.year}{last_month.month - 1:02d}"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_szse_area_summary"
