...
测试目标：
1. 验证 stock_zt_pool_dtgc_em 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：stock_zt_pool_dtgc_em
- 描述：东方财富网-行情中心-涨停板行情-跌停股池
- 输入参数：date (默认当前日期)
- 输出参数：序号、代码、涨跌幅、最新价、成交额、流通市值、总市值、动态市盈率、换手率、封单资金、最后封板时间、板上成交额、连续跌停、开板次数、所属行业
"""
...
class TestStockZtPoolDtgcEmCollection:
    """跌停股池 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 验证核心字段存在
            assert any(key in item for key in ["代码", "code"]), "应该包含代码字段"
            assert any(key in item for key in ["名称", "name", "股票名称"]), "应该包含名称字段"
            assert any(key in item for key in ["最新价", "price"]), "应该包含最新价字段"
            assert any(key in item for key in ["涨跌幅", "change"]), "应该包含涨跌幅字段"
            assert any(key in item for key in ["连续跌停", "consecutive_limit_down"]), "应该包含连续跌停字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_zt_pool_dtgc_em"
