...
测试目标：
1. 验证 stock_irm_ans_cninfo 数据集合的完整功能
...
- 描述：互动易-回答
- 输入参数：symbol (默认 "1495108801386602496", 提问者编号)
- 输出参数：股票代码, 公司简称, 问题, 回答内容, 提问者, 提问时间, 回答时间
"""
...
class TestStockIrmAnsCninfoCollection:
    """互动易-回答 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert "公司简称" in item or "name" in item, "应该包含公司简称字段"
            assert "问题" in item or "question" in item, "应该包含问题字段"
            assert "回答内容" in item or "answer" in item, "应该包含回答内容字段"
            assert "提问时间" in item or "question_time" in item, "应该包含提问时间字段"
    ...
    def test_refresh_creates_data(self, api_base_url, auth_headers, collection_name):
        ...
        # 使用示例代码
        params = {"symbol": "1495108801386602496"}
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_irm_ans_cninfo"
