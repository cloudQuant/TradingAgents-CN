"""
沪深京 A 股-实时行情数据 数据集合测试

测试目标：
1. 验证 stock_zh_a_spot_em 数据集合的完整功能
...
class TestStockZhASpotEMCollection:
    """沪深京 A 股-实时行情数据 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            assert any(key in item for key in ["名称", "name"]), "应该包含名称字段"
            assert any(key in item for key in ["最新价", "price", "close"]), "应该包含价格字段"
            assert any(key in item for key in ["涨跌幅", "pct_chg"]), "应该包含涨跌幅字段"
    ...
    def test_ui_update_data_flow(self):
        ...
        collection_url = f"{frontend_url}/stocks/collections/stock_zh_a_spot_em"
