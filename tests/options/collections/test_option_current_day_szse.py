"""
深圳证券交易所期权子网行情数据当日合约 数据集合测试

测试目标：
1. 验证 option_current_day_szse 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：option_current_day_szse
- 描述：深圳证券交易所-期权子网-行情数据-当日合约
- 输入参数：无
- 输出参数：序号, 合约编码, 合约代码, 合约简称, 标的证券简称(代码), 合约类型, 行权价, 合约单位, ...
"""
...
class TestOptionCurrentDaySzseCollection:
    """深圳证券交易所期权子网行情数据当日合约 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "option_current_day_szse"
