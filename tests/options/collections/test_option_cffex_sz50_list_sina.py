"""
中金所上证50指数所有合约返回的第一个合约为主力合约 数据集合测试

测试目标：
1. 验证 option_cffex_sz50_list_sina 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：option_cffex_sz50_list_sina
- 描述：中金所-上证50指数-所有合约
- 输入参数：无
...
class TestOptionCffexSz50ListSinaCollection:
    """中金所上证50指数所有合约返回的第一个合约为主力合约 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "option_cffex_sz50_list_sina"
