"""
新浪财经中金所上证50指数指定合约实时行情 数据集合测试

测试目标：
1. 验证 option_cffex_sz50_spot_sina 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：option_cffex_sz50_spot_sina
- 描述：新浪财经-中金所-上证50指数-指定合约-实时行情
- 输入参数：symbol (str) - 合约代码，如 "ho2303"
- 输出参数：看涨合约买量、买价、最新价、卖价、卖量、持仓量、涨跌、标识，行权价，看跌合约买量、买价、最新价、卖价、卖量、持仓量、涨跌、标识
"""
...
class TestOptionCffexSz50SpotSinaCollection:
    """新浪财经中金所上证50指数指定合约实时行情 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "option_cffex_sz50_spot_sina"
