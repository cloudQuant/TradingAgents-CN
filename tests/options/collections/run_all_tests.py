...
def discover_and_run_tests():
    ...
    for test_file in test_files:
        ...
        try:
            module = import_module(f"tests.options.collections.{module_name}")
...
def run_quick_check():
    ...
    try:
        from app.services.data_sources.options import (
            get_option_collection_definitions,
            get_registered_option_providers,
            get_registered_option_services,
        )
        
        definitions = get_option_collection_definitions()
        providers = get_registered_option_providers()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="运行期权数据集合测试")
