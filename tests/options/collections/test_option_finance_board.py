...
测试目标：
1. 验证 option_finance_board 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：option_finance_board
- 描述：金融期权行情数据
- 输入参数：symbol, end_month
- 输出参数：日期, 合约交易代码, 当前价, 涨跌幅, 前结价, 行权价, 数量
"""
...
class TestOptionFinanceBoardCollection:
    """金融期权行情数据 集合测试类"""
    ...
    def test_collection_data_fields(self, api_base_url, auth_headers, collection_name):
        ...
        if payload["items"]:
            ...
            # 前结价 -> pre_settle
            # 行权价 -> strike_price
            # 数量 -> volume/open_interest? (quantity)
            pass
