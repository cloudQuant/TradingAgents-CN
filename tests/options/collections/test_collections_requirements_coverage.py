"""
验证 options/collections 页面与需求文档 requirements 的一致性：
- 从 tests/options/requirements/*.md 中解析出声明的集合，提取集合名
- 校验 /api/options/collections 返回的数据集合包含这些集合名
- 访问前端页面 /options/collections/{name}，确认能正常打开（返回 HTTP 200）
...
# 复用现有的 parse_requirements 函数
from test_collections_page import parse_requirements


def _requirements_dir() -> str:
    # 本测试文件位于 tests/options/collections/
...
def extract_expected_collections(requirements_list: List[Dict[str, str]]) -> Dict[str, str]:
    """
    从已解析的需求文档列表中提取期权集合名称
    返回 {collection_name: doc_path} 的映射
    """
...
class TestOptionsCollectionsRequirementsCoverage:
    ...
    def _extract_from_frontend_page(self, frontend_base_url: str, logger) -> Optional[list]:
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                url = f"{frontend_base_url}/options/collections"
                ...
                if "/login" in page.url or "登录" in page.title():
                    ...
                    try:
                        ...
                        if login_button:
                            ...
                            if "/options/collections" not in page.url:
                ...
                # 尝试1: 查找包含 /options/collections/ 的链接
                links = page.query_selector_all('a[href*="/options/collections/"]')
                if not links:
                    links = page.query_selector_all('tr td a')
                
                for link in links:
                    href = link.get_attribute('href')
                    if href and '/options/collections/' in href:
                        name = href.split('/options/collections/')[-1].split('?')[0].split('#')[0]
                        if name and name not in collections:
                            collections.append(name)
                
                # 尝试2: 查找data属性中包含集合信息的元素
                if not collections:
                        ...
                        result = page.evaluate(
                            ...
                            async () => {
                                try {
                                    ...
                                    const resp = await fetch('/api/options/collections', { credentials: 'include', headers });
    ...
    def _get_collections_list(self, frontend_base_url: str, auth_headers: Dict[str, str], logger) -> Optional[list]:
        ...
        if data == "AUTH_REQUIRED" or data == "AUTH_FAILED":
            logger.log(f"    [i] 跳过前端提取（需要登录或认证失败）")
            data = None
        elif data:
            return data

        # 方式2: 尝试后端API（备选）
        backend_api_url = f"http://localhost:8000/api/options/collections"
    ...
    def test_requirements_collections_covered_by_api(self, frontend_base_url: str, auth_headers: Dict[str, str]):
        """
        要求：前端 API /api/options/collections 返回需包含 requirements 文档中声明的集合名
        """
        # 创建日志记录器
        ...
        # 使用现有的解析函数
        requirements_list = parse_requirements(req_dir)
        expected_map = extract_expected_collections(requirements_list)
        assert expected_map, "未在需求文档中解析到任何集合，请检查文档格式"
        ...
        if extra_in_api:
            ...
            for idx, name in enumerate(sorted(extra_in_api), 1):
                route = f"{frontend_base_url}/options/collections/{name}"
                logger.log(f"  {idx:3d}. [+] {name}")
    ...
    def test_requirements_collections_frontend_openable(
        ...
        """
        要求：需求文档中声明的集合详情页 /options/collections/{name} 可打开（HTTP 200）。
        若前端未启动，跳过此用例。
        """
        # 创建日志记录器
        log_file = _get_log_file_path()
        ...
        if extra_in_api:
            ...
            for idx, name in enumerate(sorted(extra_in_api), 1):
                logger.log(f"      - {idx:3d}. {name} -> {frontend_base_url}/options/collections/{name}")
            logger.log(f"  额外集合名单: {', '.join(sorted(extra_in_api))}")
        ...
        # 检查前端是否可访问
        base_ok = False
        main_url = f"{frontend_base_url}/options/collections"
        with httpx.Client(trust_env=False, timeout=60.0, follow_redirects=True) as client:
            try:
                r = client.get(main_url)
                base_ok = (200 <= r.status_code < 400) and r.url.path.startswith("/options/collections")
        ...
        with httpx.Client(trust_env=False, timeout=30.0, follow_redirects=True) as client:
            for idx, name in enumerate(target_names, 1):
                detail_url = f"{frontend_base_url}/options/collections/{name}"
                try:
                    r = client.get(detail_url)
                    ok = (200 <= r.status_code < 400) and r.url.path.startswith(f"/options/collections/{name}")
                    if not ok:
