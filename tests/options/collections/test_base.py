...
class OptionsCollectionTestBase(unittest.TestCase):
    """期权数据集合测试基类"""
    ...
    is_simple_provider: bool = True  # 是否是无参数的SimpleProvider
    required_params: List[str] = []  # 需要参数的Provider的必需参数
    
    @classmethod
    def setUpClass(cls):
        """测试类初始化"""
        cls.providers_path = project_root / "app" / "services" / "data_sources" / "options" / "providers"
        cls.services_path = project_root / "app" / "services" / "data_sources" / "options" / "services"
        cls.metadata_path = project_root / "app" / "services" / "data_sources" / "options" / "collection_metadata.py"
    ...
    def test_03_provider_class_exists(self):
        ...
        try:
            module_name = f"app.services.data_sources.options.providers.{self.collection_name}_provider"
    ...
    def test_04_service_class_exists(self):
        ...
        try:
            module_name = f"app.services.data_sources.options.services.{self.collection_name}_service"
    ...
    def test_11_provider_registered(self):
        ...
        try:
            from app.services.data_sources.options.provider_registry import get_option_provider_by_name
            
            provider_class = get_option_provider_by_name(self.collection_name)
            self.assertIsNotNone(
                provider_class,
    ...
    def test_12_service_registered(self):
        ...
        try:
            from app.services.data_sources.options.service_registry import get_option_service_by_name
            
            service_class = get_option_service_by_name(self.collection_name)
            self.assertIsNotNone(
                service_class,
...
def run_collection_test(test_class: Type[OptionsCollectionTestBase]) -> unittest.TestResult:
    """运行单个集合的测试"""
