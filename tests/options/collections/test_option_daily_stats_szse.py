"""
深圳证券交易所市场数据期权数据日度概况 数据集合测试

测试目标：
1. 验证 option_daily_stats_szse 数据集合的完整功能
2. 测试数据获取、存储、更新、展示等核心功能
3. 确保数据的正确性和完整性

接口信息：
- 接口名称：option_daily_stats_szse
- 描述：深圳证券交易所-市场数据-期权数据-日度概况
- 输入参数：date
- 输出参数：合约标的代码, 合约标的名称, 成交量, 认购成交量, 认沽成交量, 认沽/认购持仓比, 未平仓合约总数, 未平仓认购合约数, 未平仓认沽合约数, 交易日
"""
...
class TestOptionDailyStatsSzseCollection:
    """深圳证券交易所市场数据期权数据日度概况 集合测试类"""
    ...
    def collection_name(self):
        """集合名称"""
        return "option_daily_stats_szse"
