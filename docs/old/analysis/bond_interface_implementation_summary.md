...
- `BONDS_CLOSE_RETURN_SYNC_CRON`: "15 18 * *1-5" (工作日 18:15)
...
- `BONDS_COV_INFO_DETAILS_SYNC_CRON`: "20 3 * *0" (周日 03:20)

- *数据集合**: `bond_cb_profiles`

- *特点**:
- 同时支持东方财富和同花顺两个数据源
- 批量处理可转债详情（默认 50 个，可通过 limit 参数调整）
- 自动限流（每次请求间隔 0.2 秒）
...
- `BONDS_MINUTE_DATA_SYNC_CRON`: "*/5 9-15 * *1-5" (工作日 9:00-15:00，每 5 分钟)
- `BONDS_PRE_MINUTE_SYNC_ENABLED`: 启用开关
- `BONDS_PRE_MINUTE_SYNC_CRON`: "15 9* *1-5" (工作日 09:15)

- *数据集合**: `bond_minute_quotes`

- *特点**:
- 支持多种周期（1 分钟、5 分钟、15 分钟、30 分钟、60 分钟）
- 支持盘前分时数据
- 自动字段映射（中文字段 -> 英文标准字段）
