...
`combined_data` 是 `get_stock_fundamentals_unified` 工具返回的综合数据，包含了股票的基本面分析所需的所有关键信息。这个工具会根据股票类型（A 股/港股/美股）自动选择合适的数据源并返回格式化的数据。

## ⚠️ 重要：数据获取优先级

### MongoDB 优先策略

- *对于 A 股数据，系统采用 MongoDB 优先策略**：
...
1. **数据源优先级配置**
   ...
   - 支持按市场类别（A 股/港股/美股）设置不同优先级

### 为什么 MongoDB 优先？

- ✅ **性能更快**：本地数据库查询比 API 调用快 10-100 倍
- ✅ **稳定可靠**：不受 API 限流、网络波动影响
- ✅ **数据一致**：定时同步任务保证数据新鲜度
- ✅ **成本更低**：减少 API 调用次数，降低费用
- ✅ **离线可用**：即使 API 不可用也能继续分析
...
#### 2.1 中国 A 股数据 (is_china=True)

对于 A 股，`combined_data` 包含两个主要模块：
...
1. **MongoDB 优先**（如果启用 `TA_USE_APP_CACHE`）：
   ...
   - `stock_basic_info` 集合 → 基础信息（行业、板块、市值、PE、PB 等）

1. **API 数据源**（MongoDB 无数据时降级）：
   - AKShare API → 财务数据
   - Tushare API → 财务数据（AKShare 失败时）
...
- *第二优先级：API 数据源**（降级策略）
- AKShare API（默认第一 API 优先级）
- Tushare API（默认第二 API 优先级）
- BaoStock API（默认第三 API 优先级）
...
graph TD
    ...
    C1A -->|否| C1C[API 数据源<br/>Tushare/AKShare]

    C --> C2[基本面数据获取]
    C2 --> C2A{TA_USE_APP_CACHE?}
    C2A -->|是| C2B[MongoDB 优先]
...
graph TD
    A[调用 get_stock_fundamentals_unified] --> B{识别股票类型}
    B -->|A 股| C[获取 A 股数据<br/>MongoDB 优先]
...
- **A 股数据处理**: `tradingagents/dataflows/optimized_china_data.py`
  - `OptimizedChinaDataProvider` 类
  - `_get_real_financial_metrics()` - MongoDB 优先的财务数据获取
...
- **运行时配置**: `tradingagents/config/runtime_settings.py`
  - `use_app_cache_enabled()` - 检查是否启用 MongoDB 缓存
