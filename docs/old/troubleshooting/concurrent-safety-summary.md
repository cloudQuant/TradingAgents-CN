...
用户提交批量分析 3 个股票时，发现任务是顺序执行的，而不是并发执行。

## 发现的问题

经过深入分析，发现了**两个独立的 bug**：
...
🔄 [线程池] 开始执行分析: task-1 - 000001  ← 3 个任务同时开始
