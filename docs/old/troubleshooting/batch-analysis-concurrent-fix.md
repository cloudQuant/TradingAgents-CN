...
用户提交批量分析 3 个股票时，发现任务是**串行执行**的（一个接一个），而不是并发执行。
...
### 问题 1：FastAPI BackgroundTasks 是串行执行的 ⚠️ **最关键**
...
### 问题 3：TradingAgentsGraph 实例共享问题（已修复）
...
1. **app/routers/analysis.py**
   - 第 742-824 行：`submit_batch_analysis` 端点改用 `asyncio.create_task`

1. **app/services/simple_analysis_service.py**
   - 第 446-462 行：在 `__init__` 中创建共享线程池
   - 第 531-552 行：`_get_trading_graph` 每次创建新实例
   - 第 852-873 行：使用共享线程池执行分析
...
🚀 [并发任务] 开始执行: task-2 - 000002  ← 3 个任务同时开始
...
```bash
✅ TradingAgents 实例创建成功（实例 ID: 140234567890123）  ← 任务 1
✅ TradingAgents 实例创建成功（实例 ID: 140234567890456）  ← 任务 2
✅ TradingAgents 实例创建成功（实例 ID: 140234567890789）  ← 任务 3

```bash

### 4. 检查总耗时

3 个任务的总耗时应该接近单个任务的耗时，而不是 3 倍。
