...
# 在 Propagator 中设置递归限制

def get_graph_args(self) -> Dict[str, Any]:
    return {
        "stream_mode": "values",
        "config": {
            "recursion_limit": self.max_recur_limit,  # 默认 100
            "timeout": 300,  # 5 分钟超时
        },
    }
...
# 分析师团队的并行执行通过 LangGraph 自动处理

# 无需额外配置，START 节点的多个边会自动并行执行
