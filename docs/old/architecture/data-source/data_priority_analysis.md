...
# 如果启用 MongoDB 缓存，MongoDB 作为最高优先级数据源
...
1. **股票基本信息**(`get_stock_info`):
   - 第 1 优先级: MongoDB (`app_cache`) - 第 1002-1067 行
   - 第 2 优先级: Tushare/AKShare/BaoStock
   - 自动降级

2.**历史行情数据**(`get_stock_dataframe`):

   - 第 1 优先级: MongoDB - 第 534-537 行
   - 第 2 优先级: Tushare/AKShare/BaoStock
   - 自动降级 - 第 561-580 行

3.**基本面数据**(`get_fundamentals_data`):

   - 第 1 优先级: MongoDB - 第 136-137 行
   - 第 2 优先级: Tushare
   - 第 3 优先级: AKShare
   - 自动降级
...
# 第二优先级：从 AKShare API 获取

# 第三优先级：从 Tushare API 获取
...
1. **`scripts/check_mongodb_financial_data.py`**
   - 检查 MongoDB 中的财务数据
   - 验证数据结构
   - ✅ 测试通过

1. **`scripts/test_no_data_error.py`**
   - 测试无数据时的异常处理
   - 验证不使用估算值
   - ✅ 测试通过
