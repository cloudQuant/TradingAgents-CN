...
在 4 级深度分析中，Risk Manager 调用 LLM 时出现超时（Request timed out），从日志看耗时超过 6 分钟。
...
- *估算**：
- 2 轮风险讨论：每个分析师每次发言 500-1000 字符，6 次发言 = 3000-6000 字符
- 各种报告：2000-4000 字符
...
# 计算合理的超时时间：基础 300 秒 + 每轮辩论额外 60 秒

base_timeout = 300
debate_timeout = max_debate_rounds *30  # 投资辩论每轮 30 秒

risk_timeout = max_risk_discuss_rounds*60  # 风险讨论每轮 60 秒
...
| 1 级-快速 | 1 | 1 | 300 + 1×30 + 1×60 | 390 秒 |

| 2 级-基础 | 1 | 1 | 300 + 1×30 + 1×60 | 390 秒 |

| 3 级-标准 | 1 | 2 | 300 + 1×30 + 2×60 | 450 秒 |

| **4 级-深度**|**2**|**2**|**300 + 2×30 + 2×60**|**480 秒**|

|**5 级-全面**|**3**|**3**|**300 + 3×30 + 3×60**|**570 秒** |
...
```python
logger.info(f"⏱️ [Risk Manager] LLM 调用耗时: {elapsed_time:.2f}秒")
...
if hasattr(response, 'response_metadata') and 'token_usage' in response.response_metadata:
    ...
    logger.info(f"实际 Token: 输入={token_usage['prompt_tokens']} 输出={token_usage['completion_tokens']} 总计={token_usage['total_tokens']}")
...
⏱️ [Risk Manager] LLM 调用耗时: 245.32 秒
...
- *4 级深度分析**：
- Research Manager: 60-120 秒
- Risk Manager: 120-300 秒
- 总耗时: 180-420 秒（3-7 分钟）

- *5 级全面分析**：
- Research Manager: 90-180 秒
- Risk Manager: 180-450 秒
- 总耗时: 270-630 秒（4.5-10.5 分钟）
...
1. **超时错误**：
   ...
   ❌ [Risk Manager] LLM 调用失败: Request timed out.
   ```
...
1. **响应时间过长**：
   ...
   ⏱️ [Risk Manager] LLM 调用耗时: 450.00 秒
   ```
...
1. **Prompt 优化**:
   - 截断过长的辩论历史（保留最近 3000 字符）
   - 使用摘要而非完整报告
   - 减少历史记忆数量（从 2 条减到 1 条）
