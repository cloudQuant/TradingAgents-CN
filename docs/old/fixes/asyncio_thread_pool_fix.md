...
#### 1. `_get_tushare_data` 方法（2 处）

- *位置 1**: 第 773-783 行（缓存命中时获取股票信息）
...
- *位置 2**: 第 792-801 行（从 provider 获取历史数据）
...
# 方案 2: asyncio.run()（Python 3.7+，但不适合需要复用 loop 的场景）
