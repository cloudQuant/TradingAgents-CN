...
#### 系统提示（`market_analyst.py` 第 321 行）
...
#### 原始配置（`tradingagents/graph/trading_graph.py` 第 289-299 行）
...
1. LLM 绑定工具：[get_stock_market_data_unified]

   ↓

1. LLM 生成 tool_calls：{"name": "get_stock_market_data_unified", ...}

   ↓

1. should_continue_market 检测到 tool_calls

   ↓

1. tools_market (ToolNode) 执行工具
...
1. LLM 生成 tool_calls：{"name": "get_YFin_data", ...}  ❌ 错误！
...
1. tools_market (ToolNode) 执行 get_YFin_data
...
1. **添加工具调用验证**（方案 4）- 最简单，立即可用
2. **强制工具调用**（方案 2）- 确保使用正确的工具
