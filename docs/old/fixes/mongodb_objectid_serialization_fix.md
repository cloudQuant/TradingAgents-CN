...
### 方案 1: 查询时排除 `_id` 字段（推荐用于不需要 ID 的场景）
...
### 方案 2: 转换 ObjectId 为字符串（推荐用于需要 ID 的场景）
...
- `query_news()` 方法（第 323-331 行）
- `search_messages()` 方法（第 549-556 行）
...
- `query_internal_messages()` 方法（第 232-239 行）
...
1. ✅ `news_data_service.py` - 2 处修复
2. ✅ `internal_message_service.py` - 1 处修复
