...
| 6 位数字 | A 股 | `000001`, `600000` |

| 4-5 位数字 | 港股 | `0700`, `00700` |

| 4-5 位数字.HK | 港股 | `0700.HK`, `00700.HK` |

| 1-5 位字母 | 美股 | `AAPL`, `TSLA` |
...
1. `web/utils/mongodb_report_manager.py` - 添加市场类型字段

### 脚本

1. `scripts/test_market_type_fix.py` - 测试脚本（新增）
2. `scripts/migrate_add_market_type.py` - 数据迁移脚本（新增）

### 文档

1. `docs/fixes/reports-market-type-missing-fix.md` - 详细修复文档（新增）
2. `docs/fixes/SUMMARY.md` - 修复总结（新增）
3. `docs/fixes/reports-market-type-fix-complete.md` - 本文档（新增）
...
market_type = market_type_map.get(market_info.get("market", "unknown"), "A 股")
