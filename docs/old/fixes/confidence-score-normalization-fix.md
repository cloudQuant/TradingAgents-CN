...
1. **`docs/features/report-detail-metrics-enhancement.md`**
   - 更新数据结构说明
   - 更新辅助函数示例
   - 添加重要说明

1. **`docs/fixes/confidence-score-normalization-fix.md`**
   - 本文档（修复说明）
...
1. **`frontend/src/views/Stocks/Detail.vue`**
   - 使用 `fmtConf()` 函数格式化置信度
   - 第 762 行：`fmtConf(lastAnalysis.value.confidence_score)`

1. **`web/components/results_display.py`**(Streamlit Web)
