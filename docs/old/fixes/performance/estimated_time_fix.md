...
```json
{
  "elapsed_time": 80.07,        // 已用时间：80 秒（1 分 20 秒）
  "remaining_time": 279.93,     // 预计剩余：280 秒（4 分 40 秒）
  "estimated_total_time": 780   // 预计总时长：780 秒（13 分钟）❌
}
...
- *数学不一致**：
- 已用时间 + 预计剩余 = 80.07 + 279.93 = **360 秒**（6 分钟）
- 但预计总时长显示 **780 秒**（13 分钟）
...
- ✅ 使用了 Redis 的 `elapsed_time`（80 秒）
- ✅ 使用了 Redis 的 `remaining_time`（280 秒）
- ❌**没有使用** Redis 的 `estimated_total_time`（360 秒）
- ❌ 保留了 MemoryStateManager 的 `estimated_total_time`（780 秒）
...
```json
{
  "elapsed_time": 80.07,        // 已用时间：80 秒
  "remaining_time": 279.93,     // 预计剩余：280 秒
  "estimated_total_time": 360   // 预计总时长：360 秒 ✅
}
