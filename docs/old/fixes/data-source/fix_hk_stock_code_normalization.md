...
- *规则**：Yahoo Finance 期望港股代码是 **4 位数字 + .HK 后缀**。
...
统一使用与 `stock_validator.py` 相同的逻辑：**先移除前导 0，再补齐到 4 位**。
...
def _normalize_hk_symbol(self, symbol: str) -> str:
    ...
    Yahoo Finance 期望的格式：0700.HK（4 位数字）
...
# 移除前导 0，然后补齐到 4 位
        clean_code = symbol.lstrip('0') or '0'  # 如果全是 0，保留一个 0
...
INFO  | ✅ 港股数据获取成功: 0700.HK, 30 条记录  ← ✅ Yahoo Finance 成功
