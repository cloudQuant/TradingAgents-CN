...
用户报告在日志中看到 `**数据深度级别**: full`，但前端选择的是 **3 级深度（标准）**。
...
- *问题**：
1. 前端传递 `research_depth = "标准"`（对应 3 级深度）
...
- 1 级（快速） → basic
- 2 级（基础） → standard
- 3 级（标准） → standard（不是 full！）
- 4 级（深度） → full
- 5 级（全面） → comprehensive
...
| 1 级（快速） | "快速" | basic | basic | basic | ✅ 正确 |

| 2 级（基础） | "基础" | standard | standard | standard | ✅ 正确 |

| 3 级（标准） | "标准" | **full**|**full**|**full**| ❌ 错误 |

| 4 级（深度） | "深度" | detailed | detailed | detailed | ⚠️ 不一致 |

| 5 级（全面） | "全面" | comprehensive | comprehensive | comprehensive | ✅ 正确 |
...
| 3 级（标准） | "标准" |**standard**|**standard**|**standard**| ✅ 正确 |

| 4 级（深度） | "深度" |**full**|**full**|**full** | ✅ 正确 |
...
- *注意**：2 级（基础）和 3 级（标准）都映射到 `standard`，这是合理的，因为它们都是标准级别的分析，只是名称不同。
