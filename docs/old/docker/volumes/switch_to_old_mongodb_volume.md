...
| **当前运行的容器**| `tradingagents-cn_tradingagents_mongodb_data_v1` | ✅ 正在使用 | ❌ 空的（只有 3 个 LLM 配置） |

|**昨天使用的数据卷**| `tradingagents_mongodb_data` | ⚠️ 未使用 | ✅**有完整数据**（15 个 LLM 配置） |
...
- *关键点**：
- ✅ 旧数据卷 `tradingagents_mongodb_data` 包含完整的配置数据（15 个 LLM）
- ✅ 新数据卷 `tradingagents-cn_tradingagents_mongodb_data_v1` 是空的（只有 3 个 LLM）
