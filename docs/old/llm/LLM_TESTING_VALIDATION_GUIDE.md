...
def test_qianfan_api_key_config():
    ...
    if not api_key:
        print("❌ 缺少千帆 API 密钥配置: QIANFAN_API_KEY")
        return False

    if not api_key.startswith("bce-v3/"):
        print("⚠️ 千帆 API 密钥格式可能不正确，建议使用 bce-v3/ 开头的格式")
        return False

    print(f"✅ 千帆 API 密钥配置正确 (格式: {api_key[:10]}...)")
    return True
...
# 检查千帆 API 密钥配置（仅需一个密钥）

echo $QIANFAN_API_KEY

# 验证密钥格式（应该以 bce-v3/ 开头）

python -c "import os; print(f'API Key 格式: {os.getenv("QIANFAN_API_KEY", "未设置")[:10]}...')"
...
print(f"Python 编码: {sys.getdefaultencoding()}")
