...
本次更新将项目配置调整为使用 6 个经过验证的 Google AI 模型，替换了之前的临时修复方案。

## 验证的模型列表

基于实际测试结果，以下 6 个模型已验证可用：
...
- ✅ 更新默认 Google 模型为 `gemini-2.5-pro`
- ✅ 添加所有 6 个验证模型的定价配置
- ✅ 保持与现有配置结构的兼容性

### 2. Google 适配器 (`tradingagents/llm_adapters/google_openai_adapter.py`)
