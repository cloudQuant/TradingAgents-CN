...
- **后端 API 路由**: 提供 RESTful 接口
- **Web 界面**: 前端交互界面
- **CLI 工具**: 命令行工具

#### 统一 API (`tradingagents/api/stock_api.py`)
...
#### 股票 API (`tradingagents/dataflows/stock_api.py`)
...
#### Tushare 适配器 (`tradingagents/dataflows/tushare_utils.py`)
...
#### AKShare 适配器 (`tradingagents/dataflows/akshare_utils.py`)
...
#### Yahoo Finance 适配器 (`tradingagents/dataflows/yfin_utils.py`)
...
#### BaoStock 适配器 (`tradingagents/dataflows/baostock_utils.py`)
...
#### TDX 适配器 (`tradingagents/dataflows/tdx_utils.py`)
...
1. **MongoDB 数据库缓存**(stock_basic_info, market_quotes, financial_data_cache)
...
1.**中国 A 股**: Tushare → AKShare → BaoStock → TDX

1. **港股**: AKShare → Yahoo Finance → Finnhub
2. **美股**: Yahoo Finance → Finnhub
...
2. **API 限制**: 各数据源都有 API 调用频率限制，系统内置了限流机制
...
- **参数**:
  - `symbol`: 股票代码 (str) - 支持 6 位 A 股代码、港股代码、美股代码
  - `start_date`: 开始日期 (str) - 格式 'YYYY-MM-DD'
  - `end_date`: 结束日期 (str) - 格式 'YYYY-MM-DD'
  - `force_refresh`: 强制刷新缓存 (bool) - 默认 False
- **返回**: 格式化的股票数据字符串 (str)
...
- **返回**: 基本面分析报告 (str)
- **数据内容**: PE 比率、PB 比率、ROE、ROA、财务指标、行业对比等
...
- **参数**:
  - `source`: 数据源类型 (ChinaDataSource 枚举)
    - `ChinaDataSource.TUSHARE`: Tushare 数据源
    - `ChinaDataSource.AKSHARE`: AKShare 数据源
    - `ChinaDataSource.BAOSTOCK`: BaoStock 数据源
...
- *中国 A 股数据源优先级:**
1. **Tushare**(最高质量) - 专业金融数据 API

2.**AKShare**(高质量) - 开源金融数据库
3.**BaoStock**(中等质量) - 免费股票数据 API
...
3.**Finnhub** - 专业金融 API (付费)

- *美股数据源优先级:**
1. **Yahoo Finance**- 免费美股数据

2.**Finnhub**- 专业金融 API (付费)
...
export TA_CHINA_MIN_API_INTERVAL_SECONDS=0.5  # API 调用间隔

export TA_US_MIN_API_INTERVAL_SECONDS=1.0     # 美股 API 调用间隔

```bash

### 3. 数据源选择建议

- **生产环境**: 使用 Tushare (需要 token)
- **开发测试**: 使用 AKShare (免费)
- **历史数据**: 优先使用缓存
- **实时数据**: 直接调用 API
