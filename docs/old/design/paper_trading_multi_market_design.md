...
1. **仅支持 A 股**：代码使用 `_zfill_code()` 强制补齐 6 位数字
2. **简单的市价单**：即时成交，无订单簿
3. **数据库集合**：
   ...
   - `paper_orders` - 订单历史
   - `paper_trades` - 成交记录
1. **价格获取**：从 `stock_basic_info` 获取最新价
...
```javascript
{
  ...
  "available_qty": 100,        // 可用数量（考虑 T+1 限制）
...
```javascript
{
  ...
  "rules": {
    ...
    "price_limit": {
      ...
      "st_up_limit": 5.0,           // ST 股涨停 5%
      "st_down_limit": -5.0         // ST 股跌停 -5%
    },
...

{
  ...
  "rules": {
    ...
    "short_selling": {
      "enabled": true,
      "pdt_rule": true,             // Pattern Day Trader 规则
      "min_account_equity": 25000   // PDT 最低账户净值
    }
  }
}
...
class PlaceOrderRequest(BaseModel):
    code: str = Field(..., description="股票代码（支持 A 股/港股/美股）")
...
2. 后端 API 修改（支持市场识别和多货币）
3. 前端 UI 增强（显示市场类型和货币）
