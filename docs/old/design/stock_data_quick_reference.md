...
| `get_china_stock_data_unified()` | A 股 | ⭐⭐⭐⭐ | A 股专用，支持多数据源 |
...
| `get_china_stock_info_unified()` | A 股 | ⭐⭐⭐⭐⭐ | 名称、行业、市场、上市日期 |
...
| `get_china_fundamentals_cached()` | A 股 | ⭐⭐⭐⭐⭐ | 完整基本面分析报告 |

| `get_china_stock_fundamentals_tushare()` | A 股 | ⭐⭐⭐⭐ | Tushare 基本面数据 |
...
| `get_financial_data()` | A 股 | ⭐⭐⭐⭐ | 原始财务数据 |

| `get_balance_sheet()` | A 股 | ⭐⭐⭐ | 资产负债表 |

| `get_income_statement()` | A 股 | ⭐⭐⭐ | 利润表 |

| `get_cashflow_statement()` | A 股 | ⭐⭐⭐ | 现金流量表 |
...
| `get_realtime_quotes()` | A 股 | ⭐⭐⭐⭐ | 实时行情快照 |

| `get_realtime_data()` | A 股 | ⭐⭐⭐ | 单只股票实时数据 |
...
| `get_finnhub_news()` | 美股 | ⭐⭐⭐⭐ | Finnhub 新闻 |

| `get_google_news()` | 全市场 | ⭐⭐⭐ | Google 新闻搜索 |
...
```bash
export TA_CHINA_MIN_API_INTERVAL_SECONDS=0.5  # A 股 API 间隔

export TA_US_MIN_API_INTERVAL_SECONDS=1.0     # 美股 API 间隔
