...
本指南提供 TradingAgents-CN 各版本之间的升级方法，确保用户能够安全、顺利地升级到最新版本。
...
- **技术修复优化**: DashScope 适配器修复、DeepSeek 死循环修复
...
- ✅ **完全向后兼容**: v0.1.11 的所有配置继续有效
- ✅ **无需数据迁移**: 现有数据和缓存无需处理
- ✅ **API 密钥复用**: 现有的 API 密钥继续使用
- ✅ **配置保持**: 所有现有设置保持不变
...
- **多 LLM 提供商集成**: 支持 4 大提供商，60+个 AI 模型
- **模型选择持久化**: 彻底解决页面刷新配置丢失问题
- **Web 界面优化**: 320px 侧边栏，快速选择按钮
...
✅ URL 包含 ?provider=xxx&model=yyy 参数
...
- ✅ **完全向后兼容**: v0.1.10 的所有配置继续有效
- ✅ **无需数据迁移**: 现有数据和缓存无需处理
- ✅ **API 密钥复用**: 现有的 DASHSCOPE_API_KEY 等继续使用
- ✅ **配置保持**: 所有现有设置保持不变
...
# 停止本地 Streamlit、MongoDB、Redis 等服务

# 3. 配置环境变量

cp .env.example .env

# 编辑.env 文件，注意 Docker 环境的特殊配置
...
# 对比.env.old 和.env，迁移必要的配置

```bash

### 问题 3: 数据库连接失败

- *症状**: MongoDB/Redis 连接失败
...
- [ ]  **Web 界面正常访问**(<http://localhost:8501)>
- [ ] **股票分析功能正常**
  - [ ]  A 股分析 (如: 000001)
  - [ ]  美股分析 (如: AAPL)
- [ ]  **LLM 模型正常工作**
  - [ ]  DeepSeek 模型 (v0.1.7 新增)
  - [ ]  阿里百炼模型
  - [ ]  Google AI 模型
- [ ]  **数据库连接正常**
  - [ ]  MongoDB 连接
