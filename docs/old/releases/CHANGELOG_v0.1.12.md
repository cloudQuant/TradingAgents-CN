...
v0.1.12 是一个重要的功能增强版本，专注于新闻分析能力的全面提升和项目结构的优化。本版本新增了完整的智能新闻分析模块，包括多层次新闻过滤、质量评估、相关性分析等核心功能，同时修复了多个关键技术问题，并对项目结构进行了全面优化。
...
- `DASHSCOPE_ADAPTER_FIX_REPORT.md` - DashScope 适配器修复报告
- `DASHSCOPE_TOOL_CALL_DEFECTS_ANALYSIS.md` - 工具调用缺陷深度分析
- `DeepSeek 新闻分析师死循环问题分析报告.md` - 死循环问题分析
- `DeepSeek 新闻分析师死循环修复完成报告.md` - 死循环修复报告
- `LLM_TOOL_CALL_FIX_REPORT.md` - LLM 工具调用修复报告
...
│   ├── DeepSeek 新闻分析师死循环问题分析报告.md
│   ├── DeepSeek 新闻分析师死循环修复完成报告.md
...
1. **DashScope 适配器工具调用失败**- 修复参数传递和错误处理

2.**DeepSeek 新闻分析师死循环**- 实现循环检测和超时保护
3.**LLM 工具调用不稳定**- 改进绑定机制和重试逻辑
...
感谢所有为 v0.1.12 版本做出贡献的开发者和用户！特别感谢：
...
- *🌟 TradingAgents-CN v0.1.12 - 让 AI 新闻分析更智能！**
