...
TradingAgents-CN 现在提供了更精细的在线工具控制机制，您可以通过环境变量灵活配置系统的在线/离线行为，而不再依赖于特定 LLM 提供商的启用状态。
...
- `get_google_news` - Google 新闻获取
- `get_reddit_news` - Reddit 新闻获取
...
```bash
OPENAI_ENABLED=false        # 只控制 OpenAI 模型
...
通过这个新的配置系统，您可以更精确地控制 TradingAgents-CN 的在线行为，在功能需求和成本控制之间找到最佳平衡点。
