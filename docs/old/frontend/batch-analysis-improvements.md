...
- *问题**：批量分析页面的 AI 模型配置与单股分析不一致，使用硬编码的模型列表。
...
1. **供应商标识**：显示模型提供商（如 dashscope, openai 等）

2. **可搜索**：添加 `filterable` 属性，支持模型名称搜索
...
ElMessageBox.confirm(
  `✅ 批量分析任务已成功提交！\n\n📊 股票数量：${total_tasks}只\n📋 批次 ID：${batch_id}\n\n 任务正在后台执行中，最多同时执行 3 个任务，其他任务会自动排队等待。\n\n 是否前往任务中心查看进度？`,
  '提交成功',
  {
...
- *问题**：前端限制为 100 只股票，但后端限制为 10 只，不一致。
...
if (stockCodes.value.length > 100) {
  ElMessage.warning('单次批量分析最多支持 100 只股票')
  return
}
...
if (stockCodes.value.length > 10) {
  ElMessage.warning('单次批量分析最多支持 10 只股票，请减少股票数量')
  return
}
...
1. **第 367-373 行**：移除未使用的 `computed` 导入

2. **第 375-388 行**：添加 `availableModels` 变量

3. **第 425-482 行**：更新 `initializeModelSettings` 函数
   - 获取可用模型列表
   - 添加辅助函数：
...
1. **第 547-550 行**：更新批量分析数量限制（100 → 10）

2. **第 583-623 行**：优化提交成功提示
   - 使用 `ElMessageBox.confirm` 显示详细信息
   - 提供"前往任务中心"和"留在当前页面"两个选项
   - 添加并发执行机制说明

1. **第 676-809 行**：添加 `advanced-config-card` 样式
   - 橙色渐变头部（与单股分析一致）
   - 统一的配置区块样式
   - 模型配置项样式
   - 分析选项样式
