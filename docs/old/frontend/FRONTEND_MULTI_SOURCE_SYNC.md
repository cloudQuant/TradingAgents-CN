...
本文档描述了 TradingAgents-CN 项目中前端多数据源同步功能的完整实现，包括组件结构、API 集成、路由配置和用户界面设计。
...
- *问题:** URL 重复 `/api/api/sync/...`

- *原因:**
- Vite 代理配置: `/api` -> `<http://localhost:8000`>
- request.ts 中 baseURL: `/api`
- API 路径: `/api/sync/...`
...
1.**TypeScript 严格模式:**提高类型安全
2.**组合式 API:**全面使用 Vue 3 Composition API
3.**微前端架构:**支持模块化部署
4.**PWA 支持:**离线功能和推送通知
