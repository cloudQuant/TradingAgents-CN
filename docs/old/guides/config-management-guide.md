...
- 🔑 **统一 API 管理**: 支持多个 LLM 提供商的 API 密钥管理
- 🧠 **智能模型选择**: 根据任务类型自动选择最优模型
- 📄 **配置文件管理**: JSON/TOML 格式的配置文件
- 💰 **成本控制**: 实时成本监控和预算管理
- 🔄 **动态配置**: 运行时配置更新
- 🛡️ **安全管理**: 敏感信息加密存储
...
1. 在 Web 界面左侧选择 **"⚙️ 配置管理"**
2. 选择要管理的功能：
   - **模型配置**- 管理 API 密钥和模型参数
   - **定价设置**- 设置各供应商的费率
   - **使用统计**- 查看 Token 使用和成本统计
   - **系统设置**- 配置系统默认参数
...
|**Google**| gemini-pro, gemini-pro-vision | USD | Google 最新模型 |
...
- **总成本**: 指定时间内的总花费
- **总请求数**: API 调用次数
- **输入 Token**: 总输入 token 数量
- **输出 Token**: 总输出 token 数量
...
```bash
总成本 = (输入 Token 数 / 1000) × 输入单价 + (输出 Token 数 / 1000) × 输出单价
...
- *目标**: 控制月度 AI 使用成本在¥500 以内

- *操作**:
1. 设置成本警告阈值为¥16.67（500/30 天）
...
- **使用报告**: 生成详细的使用报告
- **团队管理**: 支持多用户和权限管理
- **API 集成**: 提供配置管理的 API 接口

- --

- *通过配置管理系统，您可以更好地控制 AI 使用成本，优化分析效果！** 💰📊
