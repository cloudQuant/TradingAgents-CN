...
本指南详细说明如何在 TradingAgents 系统中接入新的股票数据 SDK，包括架构设计、接入流程、代码规范和测试验证。
...
```bash
外部 SDK → tradingagents 适配器 → app 数据同步服务 → MongoDB 存储 → app 查询服务 → API/前端
...
### 步骤 1: 创建 SDK 适配器 (tradingagents 层)

在 `tradingagents/dataflows/` 目录下创建新的 SDK 适配器：
...
class NewSDKSyncService:
    """新 SDK 数据同步服务 - 负责数据库操作和业务逻辑"""

    def __init__(self):
        self.provider = NewSDKProvider()  # 使用 tradingagents 的适配器
        self.stock_service = get_stock_data_service()  # 使用 app 的数据服务

    async def sync_basic_info(self):
        """同步基础信息到数据库"""

# 1. 从 tradingagents 适配器获取标准化数据
...
### 步骤 3: 注册到数据源管理器 (tradingagents 层)
...
所有 SDK 适配器必须继承 `BaseStockDataProvider`：
...
# API 调用
            ...
            if response.status_code == 200:
                ...
                self.logger.warning(f"API 返回错误状态: {response.status_code}")
                return None
...
- [API 规范文档](../design/api_specification.md)
- [基础提供器接口](../../tradingagents/dataflows/base_provider.py)
- [示例 SDK 适配器](../../tradingagents/dataflows/example_sdk_provider.py)
- [示例同步服务](../../app/worker/example_sdk_sync_service.py)

- --

- 股票数据 SDK 接入指南 - 最后更新: 2025-09-28*
