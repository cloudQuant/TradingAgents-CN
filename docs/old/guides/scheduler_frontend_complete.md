...
1. **QuotesIngestionService.run_once**- 实时行情入库（每 30 秒）

2.**run_tushare_status_check**- Tushare 状态检查（每小时）
3.**run_tushare_basic_info_sync**- Tushare 基础信息同步（每天 2:00）
4.**BasicsSyncService.run_full_sync**- 股票基础信息同步（每天 6:30）
5.**run_tushare_quotes_sync**- Tushare 行情同步（交易日 9-15 点，每 5 分钟）
6.**run_tushare_historical_sync**- Tushare 历史数据同步（交易日 16:00）
7.**run_tushare_financial_sync**- Tushare 财务数据同步（每周日 3:00）
