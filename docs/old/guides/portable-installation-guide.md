...
1. 检查数据目录权限
2. 尝试删除 `data\mongodb\db\mongod.lock` 文件后重启
3. 如果数据损坏，可以删除 `data\mongodb\db` 目录后重新初始化
