...
├── tushare_utils.py           # Tushare 工具

├── akshare_utils.py           # AKShare 工具

├── baostock_utils.py          # BaoStock 工具
...
└── example_sdk_provider.py    # 示例 SDK 适配器
...
1. **接口不统一**: `DataSourceAdapter` vs `BaseStockDataProvider`
2. **职责混乱**: 不清楚哪层负责什么
3. **维护困难**: 修改一个数据源需要改两个地方
4. **新 SDK 接入混乱**: 不知道应该放在哪里

## 🎯 规划方案

### 方案 A: 统一到 tradingagents 层 (推荐)
...
│   ├── yahoo_provider.py        # Yahoo Finance 提供器

│   ├── finnhub_provider.py      # Finnhub 提供器

│   └── your_sdk_provider.py     # 新 SDK 提供器
...
- *1. 迁移 app/services/data_sources 到 tradingagents**:
...
# 将现有的 DataSourceAdapter 改为继承 BaseStockDataProvider
...
- 🎯**职责**: 数据同步、定时任务、业务逻辑
- 📦 **包含**: 同步服务、定时任务配置
- 🔧 **特点**: 调用 tradingagents 获取数据，写入数据库
...
```bash
外部数据源 → tradingagents 适配器 → app 同步服务 → MongoDB → app 业务服务 → API/前端
