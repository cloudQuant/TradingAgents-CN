...
TradingAgents-CN 系统已成功集成 Tushare 新闻接口，提供了业界领先的多源新闻数据获取能力。
...
1. **权限错误**
   ...
   ⚠️ Tushare 新闻接口需要单独开通权限（付费功能）
   ```

   - *解决方案**: 购买 Tushare 新闻数据权限包

1. **积分不足**

   ```

   ⚠️ Tushare 积分不足，无法获取新闻数据
   ```
...
news = await provider.get_stock_news(symbol=symbol)
await cache.set(cache_key, news, expire=3600)  # 1 小时缓存
...
- [API 接口文档](../../api/news_data_api.md)
- [数据库设计](../../design/news_data_model.md)

- --

- *Tushare 新闻接口已成功集成到 TradingAgents-CN 系统！** 🎉
