...
- *正确做法**:
- 市销率应该使用**年度营业收入**或 **TTM（最近 12 个月）营业收入**
...
def _calculate_ttm_revenue(df) -> Optional[float]:
    """
    计算 TTM（最近 12 个月）营业收入

    策略：

    1. 如果最新期是年报（12 月 31 日），直接使用年报营业收入
...
# ...
    ...
    "revenue_ttm": ttm_revenue,  # TTM 营业收入（最近 12 个月）
...
if revenue_for_ps and revenue_for_ps > 0:
    ...
    if money_cap and money_cap > 0:
        ...
        logger.debug(f"✅ 计算 PS({revenue_type}): 市值{money_cap}万元 / 营业收入{revenue_for_ps}万元 = {metrics['ps']}")
