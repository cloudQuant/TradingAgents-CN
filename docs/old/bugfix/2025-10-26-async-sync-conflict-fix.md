...
```bash
⚠️ [数据来源: MongoDB] 未找到 daily 数据: 002241，降级到其他数据源
❌ mongodb 失败，尝试备用数据源获取 daily 数据...
⚠️ [数据源优先级] 从数据库读取失败: '_asyncio.Future' object has no attribute 'get'，使用默认顺序
✅ [数据来源: 备用数据源] 降级成功获取 daily 数据: akshare
...
⚠️ [数据来源: MongoDB] 未找到 daily 数据: 002241，降级到其他数据源
❌ mongodb 失败，尝试备用数据源获取 daily 数据...
✅ [数据源优先级] 市场=A 股, 从数据库读取: ['akshare', 'tushare', 'baostock']
✅ [数据来源: 备用数据源] 降级成功获取 daily 数据: akshare
...
def get_mongo_db() -> AsyncIOMotorDatabase:
    """获取 MongoDB 数据库实例（异步）"""
    if mongo_db is None:
        raise RuntimeError("MongoDB 数据库未初始化")
    return mongo_db
