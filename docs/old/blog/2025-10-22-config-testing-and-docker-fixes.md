...
2025 年 10 月 22 日，我们针对用户反馈的实际使用问题进行了深入修复，主要集中在**配置测试功能**和 **Docker 环境适配**两个方面。通过 10 个提交，解决了 LLM 配置测试、数据库连接、Google AI 中转地址支持等关键问题，显著提升了系统在生产环境中的可用性。
...
console.log('🧪 测试 LLM 配置:', {
  ...
  API 基础 URL: config.default_base_url
})
...
# ✅ Google 官方域名：提取域名部分，让 SDK 添加 /v1beta
    ...
    logger.info(f"✅ [Google 官方] SDK 会自动添加 /v1beta 路径")
else:
...
{
  "success": false,
  "message": "google gemini-2.5-flash API 响应格式异常"
}
...
# 异常情况：没有输出内容
    if finish_reason == "MAX_TOKENS":
        return {
            "success": False,
            "message": "API 响应被截断（MAX_TOKENS），请增加 maxOutputTokens 配置"
        }
...
- *数据库配置 API URL 编码**：
- 对中文数据库名称（如 `MongoDB 主库`）进行 URL 编码
