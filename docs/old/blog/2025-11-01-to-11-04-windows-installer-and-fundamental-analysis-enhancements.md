...
- *标签**: `Windows 安装器` `便携版` `基本面分析` `总市值` `端口冲突` `LLM 配置` `多平台打包`

- --

## 📋 概述

2025 年 11 月 1 日至 4 日，我们完成了一次重要的跨平台部署和基本面分析功能增强工作。通过 **22 个提交**，实现了 Windows 绿色版（便携版）打包、Windows 安装器、基本面分析总市值数据补充、端口冲突自动检测等关键功能。本次更新显著提升了系统的易用性、跨平台兼容性和数据完整性。
...
if "total_mv" not in metrics:
    logger.info(f"📊 [总市值-第 2 层] 尝试从 stock_basic_info 获取")
    ...
    if total_mv_static is not None and total_mv_static > 0:
        ...
        logger.info(f"✅ [总市值-第 2 层成功] 总市值={total_mv_static:.2f}亿元")
    else:
...
# 从 money_cap 计算（万元转亿元）
        ...
        if money_cap is not None and money_cap > 0:
            ...
            logger.info(f"✅ [总市值-第 3 层成功] 总市值={total_mv_yi:.2f}亿元")
...
- **市盈率 TTM(PE_TTM)**: {financial_estimates.get('pe_ttm', 'N/A')}
...
# 更新缓存
        ...
        logger.info(f"✅ API Key 已更新（缓存 TTL: {self._cache_ttl}秒）")
...
2. `8777623` - arm 镜像修改了配置 (2025-10-31)
...
本次更新通过 22 个提交，实现了跨平台部署和基本面分析功能的重大增强：
