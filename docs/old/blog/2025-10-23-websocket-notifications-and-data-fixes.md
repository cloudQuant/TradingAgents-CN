...
2025 年 10 月 23 日，我们进行了一次重大的架构升级和数据修复工作。通过 25 个提交，完成了从 **SSE + Redis PubSub**到**WebSocket** 的通知系统迁移，彻底解决了困扰已久的 Redis 连接泄漏问题；同时修复了 AKShare 数据源的 `trade_date` 字段格式错误，清理了 82,631 条错误数据。此外，还完成了配置管理优化、硅基流动（SiliconFlow）大模型支持、UI 改进等多项工作。
...
1. **通知服务集成** (`app/services/notifications_service.py`):
...
1. HTTP 握手（101 Switching Protocols）
...
- *配合 Nginx 超时**：
- Nginx `proxy_read_timeout`: 3600s（1 小时）
- 后端心跳间隔: 30s
- 3600s / 30s = 120 次心跳
- 确保连接不会被超时关闭
