...
2025 年 10 月 24 日是项目开发的高产日，完成了 **31 次提交**，涵盖数据源管理、实时行情优化、基本面分析增强、Docker 构建优化等多个方面。主要亮点包括：
...
- `3e6998c` - fix: 数据源降级优先级支持市场分类（A 股/美股/港股）
...
# - true: 轮流使用 Tushare rt_k → AKShare 东方财富 → AKShare 新浪财经
...
- *提交记录**：
- `7724255` - feat: 优化 PE/PB 计算策略 - 优先使用动态 PE（基于实时股价+Tushare TTM）
- `2baa89f` - fix: 修复 PE 计算日志中变量引用错误
...
1. **缺少 TTM（Trailing Twelve Months）指标**
   - TTM 是更准确的 PE 计算方式
   - 考虑最近 12 个月的盈利
...
- *提交记录**：
- `410fd21` - feat: 基本面分析同时提供 PE 和 PE_TTM 两个指标
...
1. **`docs/configuration/quotes_ingestion_config.md`**
   - 实时行情入库服务配置指南
   - 配置项详细说明
   - 不同场景的配置方案（免费用户/付费用户/只用 AKShare）
   - 权限检测说明
   - 运行监控指南
   - 常见问题解答
...
1. **`docs/analysis/market_quotes_null_code_fix.md`**
...
1. **`docs/deployment/quick-build-reference.md`**
   - Docker 快速构建参考指南
   - 分架构构建策略
   - 构建脚本使用说明
...
QUOTES_INGEST_ENABLED=true
QUOTES_INGEST_INTERVAL_SECONDS=360  # 免费用户使用默认值（6 分钟）

# QUOTES_INGEST_INTERVAL_SECONDS=30  # 付费用户可设置为 30 秒
...
1. **访问前端任务管理页面**（`<http://localhost:5173/system/scheduler`）>
   - 查看任务名称是否显示为中文
   - 测试搜索功能
   - 测试数据源筛选
   - 测试状态筛选

1. **测试基本面分析**（`<http://localhost:5173/analysis/fundamental`）>
