...
- 可指定回溯时间（Tushare：默认 24 小时，最多 7 天）
- 可限制每只股票的新闻数量
- 支持选择性同步（仅同步新闻数据）
...
result = await sync_service.sync_news_data(
    hours_back=24,              # 回溯 24 小时
    max_news_per_stock=20       # 每只股票最多 20 条新闻
...
- ✅ 添加 Tushare 新闻数据同步功能
- ✅ 添加 AKShare 新闻数据同步功能
- ✅ 支持多新闻源（Tushare 9 个，AKShare 2 个）
- ✅ 实现智能去重
