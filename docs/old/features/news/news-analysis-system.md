...
本文档详细介绍了 TradingAgentsCN 系统中的新闻分析工具链架构、提示词设计和实现机制。
...
if is_china:
    tools = [
        ...
        toolkit.get_google_news,         # Google 新闻（中文搜索）
        toolkit.get_global_news_openai   # OpenAI 全球新闻（作为补充）
    ]
...
请特别注意：
⚠️ 如果新闻数据存在滞后（超过 2 小时），请在分析中明确说明时效性限制
✅ 优先分析最新的、高相关性的新闻事件
...
- **强制要求**: 价格影响分析（不允许回避）
- **格式要求**: 中文报告 + Markdown 表格
- **质量标准**: 详细分析 + 量化评估

#### 2.2.4 约束条件

- **时效性约束**: 优先 15-30 分钟内新闻
- **可靠性约束**: 评估新闻来源权威性
- **完整性约束**: 必须包含价格影响分析
...
- **股票类型识别**: 自动识别 A 股、港股、美股
- **数据源优化**: A 股优先中文新闻源，美股优先英文新闻源
- **模式适配**: 在线/离线模式自动切换

### 4.2 多源新闻聚合

- **专业 API**: FinnHub、Alpha Vantage 提供高质量金融新闻
- **通用 API**: NewsAPI 提供广泛新闻覆盖
- **本地化**: 中文财经新闻源支持 A 股分析
...
aggregator.high_urgency_keywords = ["破产", "收购", "FDA 批准"]
...
这个新闻分析工具链和提示词系统为 TradingAgentsCN 提供了强大的新闻分析能力，能够实时获取、处理和分析市场新闻，为投资决策提供重要参考。
