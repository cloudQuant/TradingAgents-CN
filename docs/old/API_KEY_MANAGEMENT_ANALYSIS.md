...
- *问题**：当前只返回前 8 位，应该返回前 6 位+后 6 位（如 `d1el86...j41hai0`）
...
调用 _sanitize_datasource_configs()
    ├─ 数据库有 API Key → 返回缩略版本（前 6 位 + "..." + 后 6 位）
...
- *效果**：
- ✅ 统一缩略格式：前 6 位 + "..." + 后 6 位（如 `d1el86...j41hai0`）
- ✅ 支持环境变量回退
- ✅ 用户可以区分不同的 Key
...
- *实施内容**：
1. ✅ 使用公共函数 `truncate_api_key()`（前 6 位 + "..." + 后 6 位）
...
1. ✅ `app/routers/system_config.py`
...
| 数据源配置读取 | ✅ | 支持环境变量回退，返回缩略 Key（前 6 位+...+后 6 位） |

| 数据源配置更新 | ✅ | 正确处理截断 Key、占位符、清空等场景 |

| **厂家配置读取**| ✅ |**支持环境变量回退，缩略格式统一（前 6 位+...+后 6 位）**|
...
1. ✅**厂家列表返回的缩略 Key 格式不一致**→ 已统一为前 6 位+...+后 6 位
...
1. **用户清空 MongoDB Key 的行为**
   - 保存后 MongoDB 中的 Key 被清空
   - 配置验证显示黄色（因为 .env 中有值）
   - 系统实际使用 .env 中的 Key

1. **用户填写 MongoDB Key 的行为**
