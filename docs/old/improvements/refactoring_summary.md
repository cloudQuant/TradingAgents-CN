...
统一 `app/` 中的 Web 适配层，消除代码重复，保持 `tradingagents/` 核心库的独立性。

## 📊 重构前的问题

### 架构混乱

项目中存在**两套功能完全相同的 Web 适配器**：

1. **`app/services/data_source_adapters.py`**(单文件，754 行)
...
1. **容易不一致**：修复 bug 需要在两处修改（例如最近的异步/同步调用问题）
2. **混淆使用**：不同模块使用不同实现，增加学习成本
3. **维护困难**：单文件版本 754 行，难以维护
...
- ❌ `app/services/data_source_adapters.py` (754 行，已删除)
...
- [API 架构升级文档](./API_ARCHITECTURE_UPGRADE.md)
