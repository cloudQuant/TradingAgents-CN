...
- 错误：400（DSL 校验失败）、422（参数缺失/格式）、500（内部错误）。
...
3) 放量上攻：pct_chg > 3 AND amount > 1.5x20 日均额
4) 超跌反弹：rsi14 < 30 AND close > open
5) KDJ 金叉：kdj_k cross_up kdj_d AND kdj_k < 80

说明：若“20 日均额”暂缺，可先用 rolling(amount,20).mean() 内部计算，前端避免暴露。
