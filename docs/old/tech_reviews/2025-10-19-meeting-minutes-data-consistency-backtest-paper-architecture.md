...
- PIT 一致：所有特征（新闻/社媒/财报/估值/风格因子/LLM 输出）需带版本冻结（`data_version`/`feature_version`），保证复现实验可重放。
...
- `Backtrader`：GPLv3；成熟撮合与订单生命周期；跨品种；A 股规则需扩展。
...
- Phase 1（1 周）：可用可观测的模拟交易 MVP（BrokerSim/OMS/Portfolio/SSE/基本费用与滑点）。
