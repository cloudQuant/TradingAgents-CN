...
# 安全检查
        if memory is None:
            logger.warning(f"⚠️ [DEBUG] memory 为 None，跳过历史记忆检索")
...
logger.warning(f"⚠️ [记忆系统] memory 为 None，跳过历史记忆检索")
...
```bash
研究员团队通过结构化的辩论机制，确保投资决策的全面性和客观性，是 TradingAgents 框架中连接数据分析和最终决策的关键环节。
