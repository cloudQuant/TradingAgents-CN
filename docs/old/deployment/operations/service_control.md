...
| `TUSHARE_UNIFIED_ENABLED` | `true` | Tushare 服务总开关 |
...
| `AKSHARE_UNIFIED_ENABLED` | `true` | AKShare 服务总开关 |
...
| `BAOSTOCK_UNIFIED_ENABLED` | `true` | BaoStock 服务总开关 |
...
⏱ 实时行情入库任务已启动: 每 30s
🔄 配置 Tushare 统一数据同步任务...
📅 Tushare 基础信息同步已配置: 0 2 * * *
📈 Tushare 行情同步已配置: */5 9-15 * * 1-5
...
...
4. **API 限制**: 注意各数据源的 API 调用限制，避免超限
