...
为了解决 Web 应用启动时的模块导入问题，我们更新了所有相关文档和脚本中的启动命令。
...
| `start_web.sh` | 新增 Linux/macOS 启动脚本 | ✅ 新增 |
...
| `scripts/check_api_config.py` | API 配置检查工具 | ✅ 新增 |
...
2. **路径问题**: 相对导入失败
3. **依赖问题**: Streamlit 等依赖未安装
4. **环境问题**: 虚拟环境配置不当

### 🎯 **新增功能**

1. **自动安装检查**: 脚本会自动检查项目是否已安装
2. **智能路径处理**: 自动添加项目根目录到 Python 路径
3. **依赖自动安装**: 检测并安装缺失的依赖
...
- [Web 启动问题排除](./troubleshooting/web-startup-issues.md)
- [API 配置检查](../scripts/check_api_config.py)
...
1. **✅ 解决了模块导入问题**- 用户不再需要手动设置 Python 路径
...
现在用户可以更轻松地启动和使用 TradingAgents-CN！🚀
