...
1. **优雅停止**：优先使用正常停止方式，避免数据损坏
2. **备份数据**：定期备份 `data\` 目录中的数据
3. **检查日志**：停止服务后，检查 `logs\` 目录中的日志，确认没有错误
...
1. **Graceful Stop**: Prefer normal stop method to avoid data corruption
2. **Backup Data**: Regularly backup data in `data\` directory
3. **Check Logs**: After stopping services, check logs in `logs\` directory for errors
