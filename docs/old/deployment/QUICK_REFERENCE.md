...
| 脚本 | 同步 | 构建前端 | 嵌入式 Python | 打包 ZIP | 用途 |
...
| `setup_embedded_python.ps1` | ❌ | ❌ | ✅ | ❌ | 单独安装 Python |
...
│       ├── setup_embedded_python.ps1            # 安装嵌入式 Python
...
| 构建前端 | ~2-3 分钟 | 首次较慢，后续有缓存 |

| 安装嵌入式 Python | ~5-10 分钟 | 取决于网速 |

| 打包 ZIP | ~2-3 分钟 | 取决于磁盘速度 |

| **总计（首次）**|**~15-20 分钟**| 包含所有步骤 |

|**总计（更新）**|**~5 分钟** | 跳过 Python 安装 |
