...
1. **数据来源**：PE/PB 数据来自 `stock_basic_info` 集合
...
|**分析数据流**| `tradingagents/dataflows/optimized_china_data.py` | 分析报告中的 PE/PB |

|**股票详情-基本面**| `app/routers/stocks.py` - `get_fundamentals()` | 详情页基本面快照 |

|**股票筛选**| `app/routers/screening.py` | 筛选结果中的 PE/PB |

|**自选股列表**| `app/routers/favorites.py` | 自选股的 PE/PB |
...
|**股票详情页**| `frontend/src/views/Stocks/Detail.vue` | 基本面快照显示 PE |
...
- *核心函数**：
1. `calculate_realtime_pe_pb(symbol)` - 计算实时 PE/PB
