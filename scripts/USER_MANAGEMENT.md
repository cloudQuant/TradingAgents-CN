...
这个工具集提供了通过命令行管理云子量化用户账户的功能，包括修改密码、创建用户、删除用户等操作。

## 文件说明

- `user_password_manager.py` - 核心 Python 脚本
- `user_manager.bat` - Windows 批处理文件
- `user_manager.ps1` - PowerShell 脚本
...
1. **密码安全**: 所有密码都使用 SHA256 进行哈希处理，不会以明文形式存储
