...
│   │   ├── tushare.py             # Tushare 提供器

│   │   ├── akshare.py             # AKShare 提供器

│   │   ├── baostock.py            # Baostock 提供器
...
└── optimized_china_data.py          # ⭐ 核心：优化的 A 股数据提供器
...
- *主要功能**:
- 中国市场接口（6 个函数）
  - `get_china_stock_data_unified()` - 统一的 A 股数据获取
  - `get_china_stock_info_unified()` - 统一的 A 股信息获取
  - `get_china_stock_data_tushare()` - Tushare 数据获取
  - `get_china_stock_fundamentals_tushare()` - Tushare 基本面数据
...
- 美国市场接口（7 个函数）
  ...
  - `get_YFin_data()` - Yahoo Finance 数据
  - `get_fundamentals_finnhub()` - Finnhub 基本面数据
  - 等

- 新闻接口（5 个函数）
  - `get_google_news()` - Google 新闻
  - `get_reddit_global_news()` - Reddit 全球新闻
  - `get_stock_news_openai()` - OpenAI 新闻分析
  - 等

- 技术分析接口（2 个函数）
  - `get_stockstats_indicator()` - 技术指标计算

- *使用场景**: Agent 工具函数、API 路由、业务逻辑
...
- *使用场景**:
- Agent 工具函数（agent_utils.py）
- 市场分析师（market_analyst.py）
- Web 缓存管理（cache_management.py）
...
- *职责**: 统一 DataFrame 格式，支持多数据源降级

- *主要功能**:
- `get_china_daily_df_unified()` - 统一的 A 股日线数据获取
...
- `data_source_manager`: 返回格式化字符串，适合 Agent 使用
...
- *主要功能**:
- `DataSourceConfig` 类
- 管理所有数据源的配置（Tushare/AKShare/Baostock/TDX/Finnhub 等）
- 环境变量读取
- 配置验证
...
- `config.py`: Dataflows 模块通用配置

- --

### 8. config.py (2.32 KB)

- *职责**: Dataflows 模块的通用配置管理
...
- *主要功能**:
- `save_output()` - 保存 DataFrame 到 CSV
...
1. **功能重叠**:
   - 不同文件服务不同场景，重叠是合理的
   - `data_source_manager`: Agent 场景（返回字符串）
   - `unified_dataframe`: 数据分析场景（返回 DataFrame）
   - `stock_data_service`: MongoDB → TDX 降级场景

1. **配置管理**:
   - `providers_config.py`: 数据源配置（被广泛使用，保留）
   - `config.py`: Dataflows 通用配置（保留）
...
- `docs/UTILS_CLEANUP_SUMMARY.md` - Utils 文件清理总结
- `docs/TUSHARE_ADAPTER_REFACTORING.md` - Tushare Adapter 重构总结
- `docs/ADAPTER_PROVIDER_REORGANIZATION.md` - Adapter 和 Provider 文件重组总结
- `docs/DATAFLOWS_ARCHITECTURE_ANALYSIS.md` - Dataflows 架构分析
- `docs/DATAFLOWS_CONSERVATIVE_REFACTORING.md` - Dataflows 保守优化总结
